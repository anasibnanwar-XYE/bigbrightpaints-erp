...
if [[ ! -f "$ALLOWLIST" ]]; then
  ...
  echo "[architecture-check] remediation: restore the canonical module import allowlist" >&2
  exit 1
fi
...
if [[ ! -f "$ARCH_DOC" ]]; then
  echo "[architecture-check] FAIL: missing architecture doc: $ARCH_DOC_PRIMARY." >&2
