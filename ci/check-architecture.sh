...
if [[ ! -f "$ALLOWLIST" ]]; then
  ...
  echo "[architecture-check] remediation: restore ci/architecture/module-import-allowlist.txt or update this gate with the new canonical allowlist path." >&2
  exit 1
fi

if [[ ! -f "$ROOT_DIR/agents/catalog.yaml" ]]; then
  echo "[architecture-check] FAIL: missing canonical agent catalog at $ROOT_DIR/agents/catalog.yaml" >&2
  echo "[architecture-check] remediation: restore agents/catalog.yaml so module ownership and allowlist evidence stay aligned." >&2
  exit 1
fi
...
if [[ ! -f "$ARCH_DOC" ]]; then
  echo "[architecture-check] FAIL: missing canonical architecture doc: $ARCH_DOC_PRIMARY" >&2
