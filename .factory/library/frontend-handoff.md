...
- 2026-03-07 `merge-gate-release-governance-review`: the release-gate review for the merge-gate packet confirmed there is no additional auth/admin request-body or success-response shape change beyond the already-tracked forgot-password controlled non-success path. `openapi.json` stays unchanged for this governance packet, no frontend cutover is required before base-branch review, and the explicit no-op tracker entry lives in `docs/frontend-update-v2/merge-gate-release-governance-review.md`.
