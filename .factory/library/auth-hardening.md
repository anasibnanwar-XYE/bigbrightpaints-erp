...
- As of 2026-03-06 `privileged-user-boundary-hardening`, tenant-admin out-of-scope privileged user-control attempts now fail with the existing `403` auth envelope plus `ACCESS_DENIED` audit metadata across force-reset, status disable/enable, lock/unlock, session-revoke, and MFA-disable flows, while super-admins stay on the canonical `/api/v1/superadmin/tenants/{id}/...` control plane instead of tenant-admin workflow prefixes.
...
- As of 2026-03-07 `masked-admin-lock-scope-regression-fix`, tenant-admin status, lock/unlock, session-revoke, and MFA-disable denials return the same masked `400 User not found` contract for foreign ids. Retired `suspend`, `unsuspend`, and `DELETE /api/v1/admin/users/{userId}` aliases remain absent/non-mutating.
...
- status enable/disable / lock/unlock / session revoke / MFA disable
