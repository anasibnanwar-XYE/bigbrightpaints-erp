...
- M9 runtime note (2026-04-08): the compose-backed app can serve the real application boundary while actuator health/readiness still return `503 DOWN`. During M9 validation, `GET /api/v1/auth/me` returned `403` and the runtime remained usable for API proofs even though `http://localhost:9090/actuator/health` and `/readiness` stayed degraded. Treat this as degraded-but-usable runtime state and record both facts explicitly.
- M9 audit note (2026-04-08): the public `GET /api/v1/accounting/audit/events` surface does not expose `correlationId`. For correlation validation, corroborate via `GET /api/v1/accounting/audit/transactions/{journalEntryId}` event-trail details, which do expose `correlationId`.
