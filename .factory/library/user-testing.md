...
- On 2026-03-06, passive runtime setup probes for docs-only validation showed `curl -sf http://localhost:8081/actuator/health` failing with curl exit `7` and a TCP probe to `127.0.0.1:5433` returning `ConnectionRefusedError`; for documentation-only assertions, continue validation against `docs/code-review/**` and record the runtime surface degradation in synthesis instead of blocking the round.
