...
- M7 runtime note (updated 2026-05-01): if `reset_final_validation_runtime.sh` still produces no usable seeded actors, seed a PLATFORM-scoped `ROLE_SUPER_ADMIN` directly into `app_users`/`user_roles`, authenticate through `/api/v1/auth/login`, and create isolated tenant fixtures through the current Add Client route `POST /api/v1/superadmin/tenants` with `createMode=SEND_ACTIVATION`. Use `GET /api/v1/superadmin/tenants/coa-templates` for template discovery and MailHog (`http://localhost:8025`) for activation/setup email proof. The removed flat onboarding route must not be used.
