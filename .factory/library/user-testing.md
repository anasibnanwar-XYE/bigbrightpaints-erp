...
8. To validate the **current code** on the compose-backed runtime after this milestone, restart the app with explicit overrides so it uses Flyway v2 and passes production CORS validation: `SPRING_PROFILES_ACTIVE='prod,flyway-v2' ERP_CORS_ALLOWED_ORIGINS='https://app.bigbrightpaints.com' ERP_CORS_ALLOW_TAILSCALE_HTTP_ORIGINS='true' DB_PORT=5433 docker compose up -d app`.
...
- Plain `docker compose up -d --build app` currently picks up `.env` `SPRING_PROFILES_ACTIVE=dev` and localhost HTTP CORS origins, which makes the rebuilt app fail on either Flyway history detection or prod-profile origin validation. Use the explicit override command above when you need the runtime to reflect current repository code.
