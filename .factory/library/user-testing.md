...
- `GET /api/v1/accounting/month-end/checklist?periodId={id}` and `POST /api/v1/accounting/month-end/checklist/{periodId}` currently fail on fresh M4 tenants with `400 Invalid option 'PENDING'`; treat this as an application defect rather than a harness/setup problem.
