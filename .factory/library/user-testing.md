...
- For `VAL-RESET-003`, a temporary local `BEFORE INSERT` trigger on `password_reset_tokens` is a safe way to induce a real runtime persistence failure for a dedicated test user; the current fixed runtime keeps that path on the same masked `200 OK` forgot-password success contract used for unknown users, so validation should compare both responses and rely on backend logs/metrics for the persistence-failure signal.
