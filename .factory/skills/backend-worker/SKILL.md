...
Public contract changes are not complete until the canonical contract artifacts are refreshed. That means `openapi.json` plus the matching `docs/frontend-api/*` pages whenever request/response DTOs, validation/error envelopes, or example payloads exposed by live endpoints change. If no canonical contract artifact update is needed, say why in the handoff.
