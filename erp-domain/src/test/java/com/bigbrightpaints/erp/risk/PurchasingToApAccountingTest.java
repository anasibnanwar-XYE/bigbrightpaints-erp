...
@Tag("concurrency")
@Tag("reconciliation")
class PurchasingToApAccountingTest extends AbstractIntegrationTest {
  ...
  void purchaseFlow_postsBalancedJournal_andReconcilesAp() {
    String companyCode = "RISK-AP-" + shortId();
    ...
    RawMaterial rm = ensureRawMaterial(company, accounts.get("RM_INV"), "RISK-RM");
    ...
    RiskDbAssertions.assertSupplierLedgerEntriesLinkedToJournal(
        jdbcTemplate, company.getId(), persisted.getJournalEntry().getId());
    RiskDbAssertions.assertAuditLogRecordedForJournal(
  ...
  void importedCatalogRawMaterial_purchaseFlowPreservesMaterialIdentity() {
    String companyCode = "RISK-AP-CAT-" + shortId();
    Company company = bootstrapCompany(companyCode);
    Map<String, Account> accounts = ensurePurchasingAccounts(company);
    Supplier supplier = ensureSupplier(company, accounts.get("AP"));
    String importedSku = ("RISK-RM-IMP-" + shortId()).toUpperCase(Locale.ROOT);

    CompanyContextHolder.setCompanyCode(companyCode);
    CatalogImportResponse importResponse =
        productionCatalogService.importCatalog(
            rawMaterialCatalogCsv(importedSku), "RISK-RM-IMP-IDEMP-" + shortId());
  ...
  void grnIdempotency_replayReturnsSameReceipt_andMovementsRemainStable() {
    String companyCode = "RISK-GRN-IDEMP-" + shortId();
    Company company = bootstrapCompany(companyCode);
    Map<String, Account> accounts = ensurePurchasingAccounts(company);
    Supplier supplier = ensureSupplier(company, accounts.get("AP"));
    RawMaterial rm = ensureRawMaterial(company, accounts.get("RM_INV"), "RISK-RM");
  ...
  void concurrentPurchaseInvoice_createsSinglePurchase_andSingleJournal() {
    String companyCode = "RISK-AP-CONC-" + shortId();
    Company company = bootstrapCompany(companyCode);
    Map<String, Account> accounts = ensurePurchasingAccounts(company);
    Supplier supplier = ensureSupplier(company, accounts.get("AP"));
    RawMaterial rm = ensureRawMaterial(company, accounts.get("RM_INV"), "RISK-RM");
    ...
    RiskConcurrencyHarness.RunResult<RawMaterialPurchaseResponse> result =
    ...
    assertThat(result.outcomes())
        .filteredOn(outcome -> outcome instanceof RiskConcurrencyHarness.Outcome.Success<?>)
        .hasSize(1);
  ...
  private Supplier ensureSupplier(Company company, Account apAccount) {
    return supplierRepository
        .findByCompanyAndCodeIgnoreCase(company, "RISK-SUP")
        .map(
            existing -> {
        ...
        .orElseGet(
            () -> {
              ...
              supplier.setCode("RISK-SUP");
              supplier.setName("Risk Supplier");
