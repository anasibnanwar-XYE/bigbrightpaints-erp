...
@Tag("critical")
@Tag("reconciliation")
class PeriodCloseDriftScansTest extends AbstractIntegrationTest {
  ...
  void predeployScans_areCleanForClosedPeriod() {
    String companyCode = "RISK-SCAN-CLEAN-" + System.nanoTime();
    ...
    try {
      ...
      forceClosePeriod(period.getId(), "risk scan clean request", "risk scan clean approval");
  ...
  void predeployScans_detectLatePostingAndDrift() {
    String companyCode = "RISK-SCAN-DRIFT-" + System.nanoTime();
    Company company = dataSeeder.ensureCompany(companyCode, companyCode + " Ltd");
    CompanyContextHolder.setCompanyCode(companyCode);
    try {
      LocalDate today = TestDateUtils.safeDate(company);
      ...
      AccountingPeriodDto closed =
          forceClosePeriod(period.getId(), "risk scan close request", "risk scan close approval");
  ...
  void predeployScans_detectMissingSnapshot() {
    String companyCode = "RISK-SCAN-MISSING-" + System.nanoTime();
    Company company = dataSeeder.ensureCompany(companyCode, companyCode + " Ltd");
    CompanyContextHolder.setCompanyCode(companyCode);
    try {
      LocalDate today = TestDateUtils.safeDate(company);
      ...
      forceClosePeriod(period.getId(), "risk scan missing request", "risk scan missing approval");
  ...
  void predeployScans_detectSnapshotTotalsMismatch() {
    String companyCode = "RISK-SCAN-SNAP-TOTAL-" + System.nanoTime();
    Company company = dataSeeder.ensureCompany(companyCode, companyCode + " Ltd");
    CompanyContextHolder.setCompanyCode(companyCode);
    try {
      LocalDate today = TestDateUtils.safeDate(company);
      ...
      forceClosePeriod(
          period.getId(),
          "risk scan snapshot totals request",
          "risk scan snapshot totals approval");
  ...
  private Long postJournal(
    ...
    JournalEntryRequest request =
        new JournalEntryRequest(
            "SCAN-" + System.nanoTime(), entryDate, "risk scan", null, null, Boolean.TRUE, lines);
    try {
