package com.bigbrightpaints.erp.risk.support;
...
public final class RiskDbAssertions {

  private RiskDbAssertions() {}

  public static void assertBalancedJournal(
      JournalEntryRepository journalEntryRepository, Long journalId) {
  ...
  public static void assertOneInvoicePerSlip(
    ...
    Invoice invoice =
        invoiceRepository.findByCompanyAndId(company, slip.getInvoiceId()).orElseThrow();
    assertThat(invoice.getId()).isEqualTo(slip.getInvoiceId());
    if (slip.getSlipNumber() != null) {
      List<Invoice> invoicesForOrder =
          invoiceRepository.findAllByCompanyAndSalesOrderId(company, slip.getSalesOrder().getId());
      long matchingNotes =
          invoicesForOrder.stream()
  ...
  public static void assertNoNegativeInventory(JdbcTemplate jdbcTemplate, Long companyId) {
    Integer fgNegative =
        jdbcTemplate.queryForObject(
            "select count(*) from finished_goods where company_id = ? and (current_stock < 0 or"
                + " reserved_stock < 0)",
            Integer.class,
            companyId);
    assertThat(fgNegative).as("finished_goods negative stock").isZero();

    Integer fgBatchNegative =
        jdbcTemplate.queryForObject(
            """
            select count(*)
  ...
  public static void assertSinglePayrollRun(
    ...
    Integer count =
        jdbcTemplate.queryForObject(
            ...
            periodEnd);
    assertThat(count)
        .as("single payroll run for %s %s..%s", runType, periodStart, periodEnd)
        .isEqualTo(1);
  }

  public static void assertAuditLogRecordedForJournal(
      JdbcTemplate jdbcTemplate, Long journalEntryId) {
    boolean found =
        awaitCondition(
    ...
    assertThat(found).as("accounting event entry for journal %s", journalEntryId).isTrue();
  }

  public static void assertRawMaterialMovementsLinkedToJournal(
      JdbcTemplate jdbcTemplate,
      Long companyId,
    ...
    Integer total =
        jdbcTemplate.queryForObject(
            ...
            referenceId);
    assertThat(total)
        .as("raw material movements for %s/%s", referenceType, referenceId)
        .isNotNull()
        .isGreaterThan(0);
  ...
  public static void assertDealerLedgerEntriesLinkedToJournal(
      JdbcTemplate jdbcTemplate, Long companyId, Long journalEntryId) {
    Integer count =
        jdbcTemplate.queryForObject(
            """
            select count(*)
            ...
            journalEntryId);
    assertThat(count)
        .as("dealer ledger entries for journal %s", journalEntryId)
        .isNotNull()
        .isGreaterThan(0);
  }

  public static void assertSupplierLedgerEntriesLinkedToJournal(
      JdbcTemplate jdbcTemplate, Long companyId, Long journalEntryId) {
    Integer count =
        jdbcTemplate.queryForObject(
            """
            ...
            journalEntryId);
    assertThat(count)
        .as("supplier ledger entries for journal %s", journalEntryId)
        .isNotNull()
        .isGreaterThan(0);
  }
