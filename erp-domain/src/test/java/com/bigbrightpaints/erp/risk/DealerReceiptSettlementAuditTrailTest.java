...
@Tag("concurrency")
@Tag("reconciliation")
class DealerReceiptSettlementAuditTrailTest extends AbstractIntegrationTest {
  ...
  void dealerReceipt_fullSettlement_autoClosesInvoicedOrder() {
    String companyCode = "RISK-AUTOCLOSE-" + shortId();
  ...
  void dealerReceipt_isIdempotent_underConcurrency_andWritesAuditTrail() {
    String companyCode = "RISK-AR-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
    ...
    RiskConcurrencyHarness.RunResult<JournalEntryDto> result =
    ...
    List<Long> journalIds =
        result.outcomes().stream()
            .map(
                outcome ->
                    ((RiskConcurrencyHarness.Outcome.Success<JournalEntryDto>) outcome).value())
    ...
    RiskDbAssertions.assertBalancedJournal(journalEntryRepository, journalId);
    RiskDbAssertions.assertDealerLedgerEntriesLinkedToJournal(
        jdbcTemplate, company.getId(), journalId);
    RiskDbAssertions.assertAuditLogRecordedForJournal(jdbcTemplate, journalId);
    ...
    RiskDbAssertions.assertNoNegativeInventory(jdbcTemplate, company.getId());
  ...
  void dealerSettlement_isIdempotent_underConcurrency_andRejectsMismatch() {
    String companyCode = "RISK-SETTLE-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
    ...
    RiskConcurrencyHarness.RunResult<PartnerSettlementResponse> result =
        RiskConcurrencyHarness.run(
            2,
            3,
            Duration.ofSeconds(30),
    ...
    List<Long> journalIds =
        result.outcomes().stream()
            .map(
                outcome ->
                    ((RiskConcurrencyHarness.Outcome.Success<PartnerSettlementResponse>) outcome)
                        .value())
  ...
  void dealerSettlement_isIdempotent_underConcurrency_withoutReferenceNumber() {
    String companyCode = "RISK-SETTLE-NOREF-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerSettlement_headerLevelFutureApplication_replaysWithoutDuplicateAllocations() {
    String companyCode = "RISK-SET-HDR-FUT-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerSettlement_paymentEventFirst_linksJournalAndAllocations() {
    String companyCode = "RISK-SET-PE-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerAutoSettle_usesSettlementPaymentFlow_andAllocatesOldestFirst() {
    String companyCode = "RISK-AUTO-SET-PE-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerAutoSettle_keylessSuccessiveSameAmount_settlesNextOldestInvoice() {
    String companyCode = "RISK-AUTO-SET-KEYLESS-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
    ...
    AutoSettlementRequest keylessRequest =
        new AutoSettlementRequest(
            ...
            new BigDecimal("20.00"),
            null,
            "risk keyless dealer auto-settlement",
            null);
  ...
  void
    ...
    String companyCode = "RISK-SET-REF-REUSE-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerReceiptSplit_isIdempotent_underConcurrency_andWritesAuditTrail() {
    String companyCode = "RISK-DRS-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
  ...
  void dealerReceiptSplit_zeroAllocationReplay_returnsOriginalJournal() {
    String companyCode = "RISK-DRS-ZERO-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Account cash = ensureAccount(company, "CASH", "Cash", AccountType.ASSET);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
    ...
    DealerReceiptSplitRequest request =
        new DealerReceiptSplitRequest(
            ...
            referenceNumber,
            "risk zero-allocation split receipt",
            idempotencyKey);
  ...
  void dealerReceipt_paymentEventFirst_linksJournalAndExplicitOnAccountAllocation() {
    String companyCode = "RISK-DR-PAYMENT-EVENT-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerReceipt_replayFailsClosed_whenAllocationRowsAreMissing() {
    String companyCode = "RISK-DR-MISSING-ALLOC-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerReceiptSplit_replayFailsClosed_whenAllocationRowsAreMissing() {
    String companyCode = "RISK-DRS-MISSING-ALLOC-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
    ...
    DealerReceiptSplitRequest request =
        new DealerReceiptSplitRequest(
            ...
            referenceNumber,
            "risk degraded replay split receipt",
            idempotencyKey);
  ...
  void dealerReceiptSplit_replayFailsClosed_whenOnlySubsetOfAllocationRowsSurvives() {
    String companyCode = "RISK-DRS-SUBSET-ALLOC-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Account cash = ensureAccount(company, "CASH", "Cash", AccountType.ASSET);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
    ...
    DealerReceiptSplitRequest request =
        new DealerReceiptSplitRequest(
            ...
            referenceNumber,
            "risk degraded subset split receipt",
            idempotencyKey);
  ...
  void dealerReceipt_rejectsCrossDealerInvoiceAllocation() {
    String companyCode = "RISK-DR-CROSS-DEALER-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"), "RISK-DEALER-A", "Risk Dealer A");
    Dealer foreignDealer =
        ensureDealer(company, accounts.get("AR"), "RISK-DEALER-B", "Risk Dealer B");
  ...
  void dealerReceiptSplit_autoAllocatesDeterministically_andPersistsUnappliedRemainder() {
    String companyCode = "RISK-DRS-FIFO-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Account cash = ensureAccount(company, "CASH", "Cash", AccountType.ASSET);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void dealerReceipt_reusedReferenceWithDifferentIdempotencyKey_doesNotDuplicateAllocations() {
    String companyCode = "RISK-DR-REF-REUSE-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  void
    ...
    String companyCode = "RISK-DRS-REF-REUSE-" + shortId();
    Company company = bootstrapCompany(companyCode, "UTC");
    Map<String, Account> accounts = ensureCoreAccounts(company);
    Dealer dealer = ensureDealer(company, accounts.get("AR"));
  ...
  private Dealer ensureDealer(Company company, Account arAccount) {
    return ensureDealer(company, arAccount, "RISK-DEALER", "Risk Dealer");
  }
  ...
  private void ensureCatalogProduct(Company company, FinishedGood fg, BigDecimal gstRate) {
    ProductionBrand brand =
        productionBrandRepository
            .findByCompanyAndCodeIgnoreCase(company, "RISK-BRAND")
            .orElseGet(
                () -> {
                  ProductionBrand b = new ProductionBrand();
                  b.setCompany(company);
                  b.setCode("RISK-BRAND");
                  b.setName("Risk Brand");
  ...
  private Invoice issueInvoiceForOrder(Company company, Dealer dealer, SalesOrder order) {
    ...
    invoice.setInvoiceNumber("TEST-INV-" + order.getId());
  ...
  private Invoice createStandaloneInvoice(
    ...
    invoice.setNotes("Risk standalone invoice");
