...
@Tag("critical")
@Tag("reconciliation")
class DispatchBusinessMathFuzzTest extends AbstractIntegrationTest {
  ...
  void fuzz_dispatchMath_discount_tax_inclusiveExclusive_rounding_invariantsHold() {
    ...
    String companyCode = "RISK-FUZZ-" + shortId();
    ...
    for (int i = 0; i < 25; i++) {
                      ...
                      .map(
                          line ->
                              new DispatchConfirmRequest.DispatchLine(
                                  ...
                                  "fuzz"))
                      .toList(),
                  "risk fuzz dispatch seed=" + seed + " i=" + i,
                  "risk",
                  false,
      ...
      RiskDbAssertions.assertBalancedJournal(journalEntryRepository, response.arJournalEntryId());
    ...
    var approved =
        creditLimitOverrideService.approveRequest(
            ...
            new CreditLimitOverrideDecisionRequest(
                "risk dispatch exception approved", Instant.now().plus(Duration.ofHours(4))),
            "risk-checker");
    return approved.id();
  }
