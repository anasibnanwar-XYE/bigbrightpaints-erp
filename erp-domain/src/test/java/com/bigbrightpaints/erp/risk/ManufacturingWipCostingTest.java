...
@Tag("critical")
@Tag("reconciliation")
class ManufacturingWipCostingTest extends AbstractIntegrationTest {
  ...
  void packing_postsBalancedWipAndFgJournals_andClearsWipForPackaging() {
    String companyCode = "RISK-MFG-PACK-" + shortId();
    ...
    String productionCode = "RISK-PROD-" + shortId();
    ...
    RiskDbAssertions.assertBalancedJournal(journalEntryRepository, packagingJournalId);
    RiskDbAssertions.assertBalancedJournal(journalEntryRepository, packSessionJournalId);
  ...
  void costAllocation_respectsCompanyTimezoneMonthBoundaries_andIsRetrySafe() {
    String companyCode = "RISK-MFG-COST-" + shortId();
    ...
    createFullyPackedLog(
        company, product, "RISK-BATCH-IN-" + shortId(), new BigDecimal("10"), included);
    createFullyPackedLog(
        company, product, "RISK-BATCH-OUT-" + shortId(), new BigDecimal("10"), excluded);
  ...
  private RawMaterial ensurePackagingMaterial(
    ...
    return rawMaterialRepository
        .findByCompanyAndSku(company, "RISK-BUCKET")
        .orElseGet(
            () -> {
              RawMaterial rm = new RawMaterial();
              rm.setCompany(company);
              rm.setSku("RISK-BUCKET");
              rm.setName("Bucket");
              rm.setUnitType("PCS");
