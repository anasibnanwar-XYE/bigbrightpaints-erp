...
class LegacyDispatchInvoiceLinkMatcherTest {
  ...
  void isSlipLinkedToInvoice_requiresExactMatchWhenExplicitInvoiceLinksExist() {
    ...
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                matchingSlip, invoice, List.of(matchingSlip, unrelatedSlip), 0))
        .isTrue();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                unrelatedSlip, invoice, List.of(matchingSlip, unrelatedSlip), 0))
        .isFalse();
  }
  ...
  void isSlipLinkedToInvoice_failsClosedForAmbiguousLegacyCandidates() {
    ...
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                legacySlip, invoice, List.of(legacySlip), 1))
        .isTrue();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                legacySlip, invoice, List.of(legacySlip), 2))
        .isFalse();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                legacySlip, invoice, List.of(legacySlip, otherLegacySlip), 1))
        .isFalse();
  }
  ...
  void matcherHelpers_coverNullAndExplicitLinkBranches() {
    ...
    assertThat(LegacyDispatchInvoiceLinkMatcher.hasExplicitInvoiceLinks(List.of(legacySlip)))
        .isFalse();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.hasExplicitInvoiceLinks(
                List.of(legacySlip, explicitSlip)))
        .isTrue();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                null, invoice(105L, "ISSUED"), List.of(legacySlip), 1))
        .isFalse();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                legacySlip, null, List.of(legacySlip), 1))
        .isFalse();
  }
  ...
  void isSlipLinkedToInvoice_rejectsExplicitCandidatesWhenIdsAreMissing() {
    ...
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                legacySlip, invoice(107L, "ISSUED"), List.of(explicitSlip, legacySlip), 1))
        .isFalse();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                explicitSlip, invoice(null, "ISSUED"), List.of(explicitSlip), 1))
        .isFalse();
  }
  ...
  void isSlipLinkedToInvoice_rejectsNullOrEmptyLegacyCandidateSets() {
    ...
    assertThat(LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(legacySlip, invoice, null, 1))
        .isFalse();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                legacySlip, invoice, List.of(), 1))
        .isFalse();
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.isSlipLinkedToInvoice(
                legacySlip, invoice, Arrays.asList((PackagingSlip) null), 1))
        .isFalse();
  }
  ...
  void countCurrentInvoices_handlesNullListsAndNullEntries() {
    ...
    assertThat(
            LegacyDispatchInvoiceLinkMatcher.countCurrentInvoices(
                Arrays.asList(null, invoice(118L, "VOID"))))
        .isZero();
  }
