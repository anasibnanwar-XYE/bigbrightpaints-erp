...
class MoneyUtilsTest {
    ...
    void safeMultiply_leftNull_returnsZero() {
        assertThat(MoneyUtils.safeMultiply(null, new BigDecimal("5"))).isEqualByComparingTo(BigDecimal.ZERO);
    }

    @Test
    void safeMultiply_rightNull_returnsZero() {
        assertThat(MoneyUtils.safeMultiply(new BigDecimal("5"), null)).isEqualByComparingTo(BigDecimal.ZERO);
    }
    ...
    void safeDivide_scalesAndRounds() {
        assertThat(MoneyUtils.safeDivide(new BigDecimal("10"), new BigDecimal("3"), 2, RoundingMode.HALF_UP))
                .isEqualByComparingTo(new BigDecimal("3.33"));
    }

    @Test
    void withinTolerance_exactMatch_true() {
        assertThat(MoneyUtils.withinTolerance(new BigDecimal("10"), new BigDecimal("10"), new BigDecimal("0.01")))
                .isTrue();
    }

    @Test
    void withinTolerance_withinDelta_true() {
        assertThat(MoneyUtils.withinTolerance(new BigDecimal("10.00"), new BigDecimal("10.05"), new BigDecimal("0.10")))
                .isTrue();
    }

    @Test
    void withinTolerance_outsideDelta_false() {
        assertThat(MoneyUtils.withinTolerance(new BigDecimal("10.00"), new BigDecimal("10.20"), new BigDecimal("0.10")))
                .isFalse();
    }
}
