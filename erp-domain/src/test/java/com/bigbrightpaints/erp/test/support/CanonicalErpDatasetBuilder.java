...
public class CanonicalErpDatasetBuilder {
  ...
  public CanonicalErpDatasetBuilder(
      TestDataSeeder dataSeeder,
  ...
  public CanonicalErpDataset seedCompany(String companyCode) {
    ...
    Dealer dealer =
        dealerRepository
            ...
            .orElseThrow(
                () -> new IllegalStateException("Fixture dealer missing for " + companyCode));
    Supplier supplier =
        supplierRepository
            .findByCompanyAndCodeIgnoreCase(company, "FIX-SUP")
            .orElseThrow(
                () -> new IllegalStateException("Fixture supplier missing for " + companyCode));
    ...
    ProductionBrand brand =
        brandRepository
            ...
            .orElseThrow(
                () -> new IllegalStateException("Fixture brand missing for " + companyCode));
    ProductionProduct product =
        productRepository
            .findByCompanyAndSkuCode(company, "FG-FIXTURE")
            .orElseThrow(
                () -> new IllegalStateException("Fixture product missing for " + companyCode));
    ...
    FinishedGood finishedGood =
        finishedGoodRepository
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException("Fixture finished good missing for " + companyCode));
    ensureFinishedGoodAccounts(finishedGood, accounts);

    return new CanonicalErpDataset(
        company, accounts, dealer, supplier, brand, product, finishedGood);
  }
  ...
  private void ensureProductMetadata(ProductionProduct product, Map<String, Account> accounts) {
    Map<String, Object> metadata =
        Optional.ofNullable(product.getMetadata()).orElseGet(HashMap::new);
  ...
  private void ensureFinishedGoodAccounts(
      FinishedGood finishedGood, Map<String, Account> accounts) {
    boolean updated = false;
