...
class ChangelogControllerSecurityIT extends AbstractIntegrationTest {
    ...
    @BeforeEach
    void seedUsers() {
        dataSeeder.ensureUser(ADMIN_EMAIL, ADMIN_PASSWORD, "Changelog Admin", TENANT, List.of("ROLE_ADMIN"));
        dataSeeder.ensureUser(SUPER_ADMIN_EMAIL, SUPER_ADMIN_PASSWORD, "Changelog Super", TENANT, List.of("ROLE_SUPER_ADMIN"));
        dataSeeder.ensureUser(DEALER_EMAIL, DEALER_PASSWORD, "Changelog Dealer", TENANT, List.of("ROLE_DEALER"));
    }
    ...
    void publicEndpoints_areAccessibleWithoutAuthentication() {
        ...
        ResponseEntity<Map> highlightedResponse = rest.getForEntity("/api/v1/changelog/latest-highlighted", Map.class);
