...
class InventoryAdjustmentCostingMethodTest extends AbstractIntegrationTest {
  ...
  void createAdjustment_fifoConsumesOldestBatchFirst() {
    ...
    FinishedGoodBatch oldest =
        createBatch(
            finishedGood, "FIFO-OLD", "2", "2", "10", Instant.parse("2026-01-01T00:00:00Z"));
    FinishedGoodBatch newest =
        createBatch(
            finishedGood, "FIFO-NEW", "2", "2", "20", Instant.parse("2026-01-02T00:00:00Z"));
    ...
    assertThat(adjustment.getLines().getFirst().getAmount())
        .isEqualByComparingTo(new BigDecimal("40.00"));

    FinishedGoodBatch oldestAfter =
        finishedGoodBatchRepository.findById(oldest.getId()).orElseThrow();
    FinishedGoodBatch newestAfter =
        finishedGoodBatchRepository.findById(newest.getId()).orElseThrow();
  ...
  void createAdjustment_lifoConsumesNewestBatchFirst() {
    ...
    FinishedGoodBatch oldest =
        createBatch(
            finishedGood, "LIFO-OLD", "2", "2", "10", Instant.parse("2026-01-01T00:00:00Z"));
    FinishedGoodBatch newest =
        createBatch(
            finishedGood, "LIFO-NEW", "2", "2", "20", Instant.parse("2026-01-02T00:00:00Z"));
  ...
  void createAdjustment_weightedAverageAndPeriodChangeAffectsOnlyNewMovements() {
    ...
    InventoryAdjustment januaryAdjustment =
        createAdjustment(
            company,
            finishedGood.getId(),
            accounts.get("VAR").getId(),
    ...
    InventoryAdjustment februaryAdjustment =
        createAdjustment(
            company,
            finishedGood.getId(),
            accounts.get("VAR").getId(),
    ...
    InventoryMovement januaryMovementReloaded =
        inventoryMovementRepository.findById(januaryMovement.getId()).orElseThrow();
    assertThat(januaryMovementReloaded.getUnitCost())
  ...
  private Map<String, Account> ensureAccounts(Company company) {
    Account inventory =
        ensureAccount(
            company, "FG_INV_" + company.getCode(), "Finished Goods Inventory", AccountType.ASSET);
    Account variance =
        ensureAccount(
            company, "INV_VAR_" + company.getCode(), "Inventory Variance", AccountType.EXPENSE);
    Account cogs =
        ensureAccount(company, "COGS_" + company.getCode(), "Cost of Goods Sold", AccountType.COGS);
    Account revenue =
        ensureAccount(company, "REV_" + company.getCode(), "Revenue", AccountType.REVENUE);
  ...
  private FinishedGood createFinishedGood(
      Company company, String productCode, Map<String, Account> accounts) {
  ...
  void createAdjustment_recountUpIncreasesStockAndDebitsInventory() {
    ...
    createBatch(
        finishedGood, "RECOUNT-BASE", "2", "2", "10", Instant.parse("2026-01-01T00:00:00Z"));
    ...
    assertThat(adjustment.getLines()).hasSize(1);
    assertThat(adjustment.getLines().getFirst().getUnitCost())
    ...
    JournalEntry journal =
        journalEntryRepository.findById(adjustment.getJournalEntryId()).orElseThrow();
        ...
        new InventoryAdjustmentRequest(
            ...
            List.of(
                new InventoryAdjustmentRequest.LineRequest(
                    finishedGoodId, quantity, unitCost, "costing test line")));
  ...
  private InventoryMovement findAdjustmentMovement(InventoryAdjustment adjustment) {
    return inventoryMovementRepository
        .findByReferenceTypeAndReferenceIdOrderByCreatedAtAsc(
            "ADJUSTMENT", adjustment.getReferenceNumber())
        .stream()
        .filter(
            movement ->
                "ADJUSTMENT_OUT".equals(movement.getMovementType())
