...
class InventoryValuationServiceTest {
  ...
  void resolveDispatchUnitCost_fallsBackToWeightedAverageWhenBatchCostMissing() {
    ...
    when(finishedGoodBatchRepository.calculateWeightedAverageCost(finishedGood))
        .thenReturn(new BigDecimal("19.75"));
  ...
  void resolveDispatchUnitCost_fallsBackToWeightedAverageWhenBatchExistsWithoutUnitCost() {
    ...
    when(finishedGoodBatchRepository.calculateWeightedAverageCost(finishedGood))
        .thenReturn(new BigDecimal("14.25"));
  ...
  void resolveDispatchUnitCost_usesWeightedAverageWhenCostingMethodIsWac() {
    ...
    when(finishedGoodBatchRepository.calculateWeightedAverageCost(finishedGood))
        .thenReturn(new BigDecimal("16.50"));
