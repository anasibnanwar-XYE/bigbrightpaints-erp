...
public class CompanyControllerIT extends AbstractIntegrationTest {
  ...
  void seed() {
    ...
    resetUserState(SUPER_ADMIN_EMAIL, ROOT_COMPANY_CODE);
    resetUserState(SUPER_ADMIN_EMAIL, COMPANY_CODE);
    resetUserState(HIERARCHY_SUPER_ADMIN_EMAIL, COMPANY_CODE);
    companyRepository
  ...
  private String runtimeSetting(Long companyId, String suffix) {
    return systemSettingsRepository
        .findById("tenant.runtime." + suffix + "." + companyId)
  ...
  void list_companies_as_admin_only() {
    ...
    ResponseEntity<Map> listResp =
        rest.exchange("/api/v1/companies", HttpMethod.GET, new HttpEntity<>(headers), Map.class);
  ...
  @Test
  void list_companies_returns_all_tenants_for_platform_scoped_super_admin() {
    String platformSuperAdminEmail = "platform-list-super-admin@bbp.com";
    dataSeeder.ensureUser(
        ...
        java.util.List.of("ROLE_SUPER_ADMIN"));
    String token = loginToken(platformSuperAdminEmail, AuthScopeService.DEFAULT_PLATFORM_AUTH_CODE);
    HttpHeaders headers = new HttpHeaders();
    headers.setBearerAuth(token);
    headers.setContentType(MediaType.APPLICATION_JSON);
    headers.set("X-Company-Code", AuthScopeService.DEFAULT_PLATFORM_AUTH_CODE);
  ...
  @Test
  void support_warning_endpoint_issues_warning_for_platform_scoped_super_admin() {
    Long companyId = companyRepository.findByCodeIgnoreCase(COMPANY_CODE).orElseThrow().getId();
    String platformSuperAdminEmail = "platform-warning-super-admin@bbp.com";
    dataSeeder.ensureUser(
        platformSuperAdminEmail,
        ADMIN_PASSWORD,
        "Platform Warning Super Admin",
        AuthScopeService.DEFAULT_PLATFORM_AUTH_CODE,
        java.util.List.of("ROLE_SUPER_ADMIN"));
    String platformToken = loginToken(platformSuperAdminEmail, AuthScopeService.DEFAULT_PLATFORM_AUTH_CODE);
    HttpHeaders headers = new HttpHeaders();
  ...
  void support_admin_password_reset_revokes_existing_admin_tokens() {
    ...
    ResponseEntity<Map> resetResponse =
        rest.exchange(
            ...
            HttpMethod.POST,
            new HttpEntity<>(
                Map.of("adminEmail", ADMIN_EMAIL, "reason", "Security hard reset"), supportHeaders),
            Map.class);
    ...
    ResponseEntity<Map> meResponse =
        rest.exchange("/api/v1/auth/me", HttpMethod.GET, new HttpEntity<>(adminHeaders), Map.class);
  ...
  void retired_superadmin_tenant_aliases_are_not_exposed() {
    ...
    ResponseEntity<Map> createAliasResponse =
        rest.exchange(
            ...
            new HttpEntity<>(
                ...
                headers),
            Map.class);
    ResponseEntity<Map> updateAliasResponse =
        rest.exchange(
    ...
    assertThat(createAliasResponse.getStatusCode())
        .isIn(HttpStatus.NOT_FOUND, HttpStatus.METHOD_NOT_ALLOWED);
    assertThat(updateAliasResponse.getStatusCode())
        .isIn(HttpStatus.NOT_FOUND, HttpStatus.METHOD_NOT_ALLOWED);
    assertThat(companyRepository.findByCodeIgnoreCase(newCompanyCode)).isEmpty();
  }
  ...
  void tenant_configuration_update_allows_super_admin_outside_target_tenant_membership() {
    ...
    dataSeeder.ensureUser(
        rootOnlySuperAdminEmail,
        ADMIN_PASSWORD,
        "Root Only Super Admin",
        ROOT_COMPANY_CODE,
        java.util.List.of("ROLE_SUPER_ADMIN", "ROLE_ADMIN"));
    Map<String, Object> updateRequest =
        Map.of(
            "name",
            "Config Updated By Root",
            "code",
            COMPANY_CODE,
            "timezone",
            "UTC",
  ...
  void tenant_runtime_policy_update_rejects_invalid_payloads_without_persisting_policy() {
    ...
    ResponseEntity<Map> baselineResponse =
        rest.exchange(
            "/api/v1/companies/" + companyId + "/tenant-runtime/policy",
            HttpMethod.PUT,
            new HttpEntity<>(
  ...
  @Test
  void
      tenant_runtime_policy_update_rejects_foreign_and_unknown_targets_without_persisting_policy() {
    Long companyId = companyRepository.findByCodeIgnoreCase(COMPANY_CODE).orElseThrow().getId();
    ...
    ResponseEntity<Map> foreignTarget =
        rest.exchange(
            "/api/v1/companies/" + companyId + "/tenant-runtime/policy",
            HttpMethod.PUT,
            new HttpEntity<>(
                Map.of("holdState", "BLOCKED", "reasonCode", "foreign-target"), rootAdminHeaders),
            Map.class);
    ResponseEntity<Map> unknownTarget =
        rest.exchange(
            "/api/v1/companies/999999/tenant-runtime/policy",
            HttpMethod.PUT,
            new HttpEntity<>(
                Map.of("holdState", "BLOCKED", "reasonCode", "unknown-target"), rootAdminHeaders),
            Map.class);
  ...
  void retired_admin_tenant_runtime_policy_writer_is_not_found_and_does_not_persist() {
    ...
    ResponseEntity<Map> retiredWriterResponse =
        rest.exchange(
  ...
  void root_only_super_admin_can_recover_blocked_tenant_while_non_super_admin_cannot() {
    ...
    ResponseEntity<Map> blockResponse =
        rest.exchange(
            ...
            HttpMethod.PUT,
            new HttpEntity<>(
                Map.of("holdState", "BLOCKED", "reasonCode", "RECOVERY_DRILL_BLOCK"),
                superAdminHeaders),
            Map.class);
    assertThat(blockResponse.getStatusCode()).isEqualTo(HttpStatus.OK);

    ResponseEntity<Map> blockedMe =
        rest.exchange(
            "/api/v1/auth/me", HttpMethod.GET, new HttpEntity<>(companyAdminHeaders), Map.class);
    ...
    rootAdminHeaders.setContentType(MediaType.APPLICATION_JSON);
    rootAdminHeaders.set("X-Company-Code", ROOT_COMPANY_CODE);
    ResponseEntity<Map> deniedRecovery =
        rest.exchange(
            "/api/v1/companies/" + companyId + "/tenant-runtime/policy",
            HttpMethod.PUT,
            new HttpEntity<>(
                Map.of("holdState", "ACTIVE", "reasonCode", "UNAUTHORIZED_RECOVERY"),
                rootAdminHeaders),
            Map.class);
    ...
    ResponseEntity<Map> recoveredMe =
        rest.exchange(
            "/api/v1/auth/me", HttpMethod.GET, new HttpEntity<>(companyAdminHeaders), Map.class);
  ...
  @Test
  void
      canonical_tenant_runtime_policy_update_tracks_policy_control_and_blocks_same_node_immediately() {
    Long companyId = companyRepository.findByCodeIgnoreCase(COMPANY_CODE).orElseThrow().getId();
    String rootOnlySuperAdminEmail = "root-policy-super-admin@bbp.com";
    dataSeeder.ensureUser(
        rootOnlySuperAdminEmail,
        ADMIN_PASSWORD,
        "Root Policy Super Admin",
        ROOT_COMPANY_CODE,
        java.util.List.of("ROLE_SUPER_ADMIN"));
    ...
    ResponseEntity<Map> warmResponse =
        rest.exchange("/api/v1/auth/me", HttpMethod.GET, new HttpEntity<>(adminHeaders), Map.class);
    ...
    ResponseEntity<Map> blockedMeResponse =
        rest.exchange("/api/v1/auth/me", HttpMethod.GET, new HttpEntity<>(adminHeaders), Map.class);
