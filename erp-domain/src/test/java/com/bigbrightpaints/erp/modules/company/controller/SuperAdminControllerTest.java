...
class SuperAdminControllerTest {
    ...
    void updateTenantModules_delegatesToCompanyService() {
        CompanyEnabledModulesDto modulesDto = new CompanyEnabledModulesDto(15L, "ACME", Set.of("PORTAL"));
        when(companyService.updateEnabledModules(15L, Set.of("PORTAL"))).thenReturn(modulesDto);

        ResponseEntity<ApiResponse<CompanyEnabledModulesDto>> response = superAdminController.updateTenantModules(
                15L,
                new SuperAdminController.TenantModulesUpdateRequest(Set.of("PORTAL")));
