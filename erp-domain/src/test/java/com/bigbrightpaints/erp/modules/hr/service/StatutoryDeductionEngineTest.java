...
class StatutoryDeductionEngineTest {
  ...
  void calculateEsiDeduction_appliesOnlyBelowEligibilityThreshold() {
    ...
    BigDecimal eligibleDeduction =
        engine.calculateEsiDeduction(new BigDecimal("20000.00"), employee);
    BigDecimal ineligibleDeduction =
        engine.calculateEsiDeduction(new BigDecimal("22000.00"), employee);
  ...
  void calculateTdsDeduction_projectsAnnualGrossByRunTypeAndTaxRegime() {
    ...
    BigDecimal monthlyTds =
        engine.calculateTdsDeduction(new BigDecimal("30000.00"), monthlyRun, employee);
    BigDecimal weeklyTds =
        engine.calculateTdsDeduction(new BigDecimal("6000.00"), weeklyRun, employee);
