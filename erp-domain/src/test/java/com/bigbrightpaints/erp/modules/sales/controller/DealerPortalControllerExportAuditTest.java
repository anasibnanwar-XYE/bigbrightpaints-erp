...
class DealerPortalControllerExportAuditTest {
  ...
  void createCreditLimitRequest_scopesSubmissionToAuthenticatedDealer() {
    ...
    when(creditLimitRequestService.createRequest(any(), eq(4401L), eq("dealer.user@bbp.com")))
        .thenReturn(created);

    var response =
        controller.createCreditLimitRequest(
    ...
    ArgumentCaptor<com.bigbrightpaints.erp.modules.sales.dto.CreditLimitRequestCreateRequest>
        requestCaptor =
            ArgumentCaptor.forClass(
                com.bigbrightpaints.erp.modules.sales.dto.CreditLimitRequestCreateRequest.class);
    verify(creditLimitRequestService)
        .createRequest(requestCaptor.capture(), eq(4401L), eq("dealer.user@bbp.com"));
  ...
  void getMyInvoicePdf_logsDataExportMetadata() {
    ...
    InvoicePdfService.PdfDocument pdf =
        new InvoicePdfService.PdfDocument("dealer-invoice-88.pdf", pdfBytes);
    ...
    assertThat(response.getHeaders().getFirst(HttpHeaders.CONTENT_DISPOSITION))
        .contains("dealer-invoice-88.pdf");
