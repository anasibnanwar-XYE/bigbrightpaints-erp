...
class AccountingEventStoreMetricsTest {
    ...
    void recordJournalEntryPosted_incrementsBusinessJournalMetrics() {
        ...
        AccountingEventStore store = new AccountingEventStore(
                eventRepository,
                eventPublisher,
                new ObjectMapper(),
                companyClock,
        ...
        when(eventRepository.save(any(AccountingEvent.class))).thenAnswer(invocation -> invocation.getArgument(0));

        store.recordJournalEntryPosted(entry, Map.of(101L, new BigDecimal("1000.00"), 202L, new BigDecimal("250.00")));

        assertThat(meterRegistry.get("erp.business.journals.created").counter().count()).isEqualTo(1.0d);
        assertThat(meterRegistry.get("erp.business.journals.created.by_company")
                .tag("company", "ACME")
                .counter()
    ...
    private JournalEntry buildJournalEntry() {
        ...
        ReflectionTestUtils.setField(entry, "publicId", UUID.fromString("11111111-1111-1111-1111-111111111111"));
