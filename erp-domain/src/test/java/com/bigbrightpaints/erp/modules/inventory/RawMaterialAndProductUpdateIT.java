...
class RawMaterialAndProductUpdateIT extends AbstractIntegrationTest {
  ...
  void catalog_raw_material_item_class_creates_linked_material_and_product() {
    ...
    ProductionProduct linked =
        productionProductRepository.findByCompanyAndSkuCode(company, sku).orElseThrow();

    assertThat(sku).startsWith("RM-");
    assertThat(((Number) createdData.get("rawMaterialId")).longValue())
        .isEqualTo(rawMaterial.getId());
  ...
  void catalog_product_update_rejects_reclassifying_raw_material_to_packaging() {
    ...
    RawMaterial updatedMaterial =
        rawMaterialRepository.findByCompanyAndId(company, rawMaterialId).orElseThrow();
  ...
  void catalog_raw_material_item_class_rejects_malformed_costing_token() {
    ...
    payload.put(
        "metadata", Map.of("inventoryAccountId", accounts.get("INV"), "costingMethod", "WAC;DROP"));
  ...
  void catalog_product_update_uses_canonical_host_and_pre_resolved_brand_id() {
    ...
    ResponseEntity<Map> brandCreate =
        rest.exchange(
