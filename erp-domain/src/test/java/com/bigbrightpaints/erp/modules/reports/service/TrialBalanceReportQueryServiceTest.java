...
class TrialBalanceReportQueryServiceTest {
  ...
  void generate_forDateRangeAggregatesDebitsCreditsAndBalanceCheck() {
    ...
    FinancialReportQueryRequest request =
        new FinancialReportQueryRequest(
            null, window.startDate(), window.endDate(), null, null, null, null, null, "PDF");
    ...
    when(accountRepository.findByCompanyOrderByCodeAsc(window.company()))
        .thenReturn(List.of(cash, sales));
    when(journalLineRepository.summarizeByAccountWithin(
            window.company(), window.startDate(), window.endDate()))
        .thenReturn(List.of(row(1L, "150.00", "10.00"), row(2L, "0.00", "140.00")));
  ...
  void generate_withComparativeRangeReturnsSideBySideComparativeSnapshot() {
    ...
    when(accountRepository.findByCompanyOrderByCodeAsc(primary.company()))
        .thenReturn(List.of(cash, sales));
    when(accountRepository.findByCompanyOrderByCodeAsc(comparative.company()))
        .thenReturn(List.of(cash, sales));

    when(journalLineRepository.summarizeByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(List.of(row(11L, "200.00", "20.00"), row(12L, "0.00", "180.00")));
    when(journalLineRepository.summarizeByAccountWithin(
            comparative.company(), comparative.startDate(), comparative.endDate()))
        .thenReturn(List.of(row(11L, "120.00", "20.00"), row(12L, "0.00", "100.00")));
  ...
  void generate_asOfUsesUpToSummaryAndSkipsMalformedRows() {
    ...
    FinancialReportQueryRequest request =
        new FinancialReportQueryRequest(null, null, null, asOfDate, null, null, null, null, "CSV");
    ...
    when(accountRepository.findByCompanyOrderByCodeAsc(window.company()))
        .thenReturn(List.of(cash, payables));
    when(journalLineRepository.summarizeByAccountUpTo(window.company(), window.asOfDate()))
        .thenReturn(
  ...
  void generate_withPartialDateRangeFallsBackToAsOfSummary() {
    ...
    FinancialReportQueryRequest request =
        new FinancialReportQueryRequest(
            null, window.startDate(), window.endDate(), asOfDate, null, null, null, null, "CSV");
