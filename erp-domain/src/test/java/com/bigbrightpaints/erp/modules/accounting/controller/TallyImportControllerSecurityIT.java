...
class TallyImportControllerSecurityIT extends AbstractIntegrationTest {
  ...
  @Autowired private TallyImportRepository tallyImportRepository;

  @BeforeEach
  void setUpUsersAndDefaults() {
    dataSeeder.ensureUser(
        ADMIN_EMAIL, PASSWORD, "Tally Admin", COMPANY_CODE, List.of("ROLE_ADMIN"));
    dataSeeder.ensureUser(
        ACCOUNTING_EMAIL, PASSWORD, "Tally Accounting", COMPANY_CODE, List.of("ROLE_ACCOUNTING"));
  ...
  void tallyImport_allowsAdminOnly() {
    ResponseEntity<Map> adminResponse =
        importTally(
            authHeaders(ADMIN_EMAIL, PASSWORD, COMPANY_CODE), sampleTallyXml(), "tally-admin.xml");
  ...
  void tallyImport_sameFileReplaysByHashAndCreatesSingleImportRecord() {
    ...
    TallyImport importRecord =
        tallyImportRepository.findByCompanyAndIdempotencyKey(company, expectedHash).orElseThrow();
