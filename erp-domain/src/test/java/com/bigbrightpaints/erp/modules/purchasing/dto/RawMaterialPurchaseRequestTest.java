...
class RawMaterialPurchaseRequestTest {
  ...
  void constructorAndAccessors_preserveProvidedValues() {
    RawMaterialPurchaseLineRequest line =
  ...
  void validation_reportsTopLevelAndNestedViolations() {
    RawMaterialPurchaseLineRequest invalidLine =
        new RawMaterialPurchaseLineRequest(
            null, null, BigDecimal.ZERO, null, new BigDecimal("-1"), null, null, null);
    RawMaterialPurchaseRequest request =
        new RawMaterialPurchaseRequest(
            null, " ", null, null, null, null, new BigDecimal("-0.01"), List.of(invalidLine));
  ...
  void validation_rejectsEmptyLines() {
    ...
    assertThat(violations).extracting(v -> v.getPropertyPath().toString()).contains("lines");
  }
}
