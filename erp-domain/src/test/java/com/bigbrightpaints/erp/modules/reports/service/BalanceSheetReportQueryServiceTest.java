...
class BalanceSheetReportQueryServiceTest {
  ...
  void generate_groupsCurrentAndLongTermSectionsAndValidatesEquation() {
    BalanceSheetReportQueryService service =
    ...
    FinancialReportQueryRequest request =
        new FinancialReportQueryRequest(
            null, primary.startDate(), primary.endDate(), null, null, null, null, null, "CSV");
    ...
    Account loan =
        account(4L, "LONG-TERM-BORROWINGS", "Long-Term Borrowings", AccountType.LIABILITY);
    ...
    when(accountRepository.findByCompanyOrderByCodeAsc(primary.company()))
        .thenReturn(List.of(cash, machinery, payable, loan, equity));
    ...
    assertThat(dto.currentAssets())
        .extracting(BalanceSheetDto.SectionLine::accountCode)
        .containsExactly("CASH");
    assertThat(dto.fixedAssets())
        .extracting(BalanceSheetDto.SectionLine::accountCode)
        .containsExactly("FIXED-ASSETS");
    assertThat(dto.currentLiabilities())
        .extracting(BalanceSheetDto.SectionLine::accountCode)
        .containsExactly("AP");
    assertThat(dto.longTermLiabilities())
        .extracting(BalanceSheetDto.SectionLine::accountCode)
        .containsExactly("LONG-TERM-BORROWINGS");
    assertThat(dto.equityLines())
        .extracting(BalanceSheetDto.SectionLine::accountCode)
  ...
  void generate_rollsCurrentPeriodEarningsIntoLiveEquity() {
    ...
    FinancialReportQueryRequest request =
        new FinancialReportQueryRequest(
            null, primary.startDate(), primary.endDate(), null, null, null, null, null, null);
    ...
    when(journalLineRepository.summarizeByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(List.of(row(1L, "90.00", "0.00"), row(2L, "0.00", "30.00")));
  ...
  void generate_skipsCurrentEarningsForClosedSnapshot() {
    ...
    ReportQuerySupport.FinancialQueryWindow snapshotWindow =
    ...
    var line =
        new com.bigbrightpaints.erp.modules.accounting.domain.AccountingPeriodTrialBalanceLine();
  ...
  void generate_excludesPeriodCloseJournalsFromLiveRangeSummary() {
    ...
    ReportQuerySupport.FinancialQueryWindow primary =
        new ReportQuerySupport.FinancialQueryWindow(
            ReportFixtures.window(
                    LocalDate.of(2026, 3, 15), LocalDate.of(2026, 3, 31), LocalDate.of(2026, 3, 31))
                .company(),
    ...
    FinancialReportQueryRequest request =
        new FinancialReportQueryRequest(
            44L, primary.startDate(), primary.endDate(), null, null, null, null, null, null);
    ...
    when(journalLineRepository.summarizeByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(
            List.of(row(1L, "100.00", "0.00"), row(2L, "0.00", "40.00"), row(3L, "0.00", "60.00")));
    when(journalLineRepository.summarizePostedPeriodCloseSystemJournalsByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
    ...
    verify(journalLineRepository)
        .summarizeByAccountWithin(primary.company(), primary.startDate(), primary.endDate());
    verify(journalLineRepository)
        .summarizePostedPeriodCloseSystemJournalsByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate());
  }
  ...
  void generate_excludesPeriodCloseJournalsForMultiMonthLiveRange() {
    ...
    ReportQuerySupport.FinancialQueryWindow primary =
        ReportFixtures.window(
            LocalDate.of(2026, 1, 1), LocalDate.of(2026, 3, 31), LocalDate.of(2026, 3, 31));
    ...
    when(accountRepository.findByCompanyOrderByCodeAsc(primary.company()))
        .thenReturn(List.of(cash, retained));
    when(journalLineRepository.summarizeByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(
            java.util.List.<Object[]>of(row(1L, "120.00", "0.00"), row(2L, "0.00", "120.00")));
    when(journalLineRepository.summarizePostedPeriodCloseSystemJournalsByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(java.util.List.<Object[]>of(row(2L, "0.00", "120.00")));
    when(journalLineRepository.summarizeByAccountType(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(
  ...
  void generate_preservesNonSystemPeriodClosePrefixedJournalsInLiveRangeSummary() {
    ...
    when(journalLineRepository.summarizeByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(
            java.util.List.<Object[]>of(row(1L, "100.00", "0.00"), row(2L, "0.00", "140.00")));
    when(journalLineRepository.summarizePostedPeriodCloseSystemJournalsByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(
  ...
  void generate_ignoresMalformedCurrentEarningsRowsAndSupportsIncomeAndExpenseFamilies() {
    ...
    ReportQuerySupport.FinancialQueryWindow primary =
        ReportFixtures.window(
            LocalDate.of(2026, 4, 1), LocalDate.of(2026, 4, 30), LocalDate.of(2026, 4, 30));
    ...
    when(accountRepository.findByCompanyOrderByCodeAsc(primary.company()))
        .thenReturn(List.of(cash, equity));
    when(journalLineRepository.summarizeByAccountWithin(
            primary.company(), primary.startDate(), primary.endDate()))
        .thenReturn(
            java.util.Arrays.asList(
  ...
  void generate_omitsCurrentEarningsLineWhenLiveWindowNetIsZero() {
    ...
    ReportQuerySupport.FinancialQueryWindow primary =
        ReportFixtures.window(
            LocalDate.of(2026, 5, 1), LocalDate.of(2026, 5, 31), LocalDate.of(2026, 5, 31));
  ...
  void helper_branches_cover_snapshot_and_period_close_shortCircuits() {
    ...
    assertThat(
            (Boolean)
                com.bigbrightpaints.erp.test.support.ReflectionFieldAccess.invokeMethod(
                    service, "isBalanceSheetType", AccountType.ASSET))
        .isTrue();
    assertThat(
            (Boolean)
                com.bigbrightpaints.erp.test.support.ReflectionFieldAccess.invokeMethod(
