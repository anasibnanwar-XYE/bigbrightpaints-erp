...
class InventoryBatchQueryServiceTest extends AbstractIntegrationTest {
  ...
  @Autowired private InventoryBatchQueryService inventoryBatchQueryService;
  ...
  void listExpiringSoonBatches_returnsCombinedSortedRowsWithinWindow() {
    ...
    RawMaterial rawMaterial =
        createRawMaterial(company, "RM-EXP-" + shortId(), inventoryAccount.getId());
    FinishedGood finishedGood =
        createFinishedGood(company, "FG-EXP-" + shortId(), inventoryAccount.getId());
    ...
    createFinishedBatch(
        finishedGood, "FG-NEAR", LocalDate.now().plusDays(6), new BigDecimal("4.00"));
    createFinishedBatch(
        finishedGood, "FG-OUT", LocalDate.now().plusDays(60), new BigDecimal("4.00"));
    ...
    assertThat(rows).allMatch(row -> row.daysUntilExpiry() >= 0 && row.daysUntilExpiry() <= 30);
    assertThat(rows).allMatch(row -> row.expiryDate() != null);
  }
  ...
  void listExpiringSoonBatches_treatsNegativeDaysAsZero() {
    ...
    RawMaterial rawMaterial =
        createRawMaterial(company, "RM-EXP-NEG-" + shortId(), inventoryAccount.getId());
  ...
  private Account ensureInventoryAccount(Company company) {
    Account account =
        accountRepository
            .findByCompanyAndCodeIgnoreCase(company, "INV-EXP-" + company.getCode())
            .orElseGet(
                () -> {
  ...
  private void createRawBatch(
      RawMaterial material, String code, LocalDate expiryDate, BigDecimal quantity) {
  ...
  private void createFinishedBatch(
      FinishedGood finishedGood, String code, LocalDate expiryDate, BigDecimal quantity) {
