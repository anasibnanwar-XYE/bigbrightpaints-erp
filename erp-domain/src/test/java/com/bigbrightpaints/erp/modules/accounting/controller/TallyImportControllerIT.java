...
class TallyImportControllerIT extends AbstractIntegrationTest {
  ...
  void setUpUsersAndDefaults() {
    dataSeeder.ensureUser(
        ADMIN_EMAIL, PASSWORD, "Tally IT Admin", COMPANY_CODE, List.of("ROLE_ADMIN"));
    dataSeeder.ensureUser(
        SUPER_ADMIN_EMAIL,
        PASSWORD,
        "Tally IT Super Admin",
        COMPANY_CODE,
  ...
  void admin_canImportTallyData() {
    ResponseEntity<Map> response =
        importTally(authHeaders(ADMIN_EMAIL), sampleTallyXml(), "tally-admin.xml");
  ...
  void superAdmin_cannotExecuteTenantTallyImportWorkflow() {
    ResponseEntity<Map> response =
        importTally(authHeaders(SUPER_ADMIN_EMAIL), sampleTallyXml(), "tally-superadmin.xml");
