...
class AgedDebtorsReportQueryServiceTest {
  ...
  void generate_groupsOutstandingByAllAgingBucketsAndIncludesMetadataHints() {
    AgedDebtorsReportQueryService service =
        new AgedDebtorsReportQueryService(reportQuerySupport, dealerLedgerRepository);
    ...
    when(dealerLedgerRepository.findAllUnpaidAsOf(company, window.asOfDate()))
        .thenReturn(
            List.of(
    ...
    when(dealerLedgerRepository.findAllUnpaidAsOf(company, window.asOfDate()))
        .thenReturn(
            List.of(
                ...
                ledger(
                    dealer, "300.00", window.asOfDate().minusDays(45), LocalDate.of(2026, 2, 20))));
  ...
  private DealerLedgerEntry ledger(
      Dealer dealer, String outstanding, LocalDate dueDate, LocalDate entryDate) {
