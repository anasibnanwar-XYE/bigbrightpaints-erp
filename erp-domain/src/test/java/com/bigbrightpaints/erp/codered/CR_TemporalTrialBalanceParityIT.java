...
class CR_TemporalTrialBalanceParityIT extends AbstractIntegrationTest {
    ...
    void openPeriodTemporalTrialBalance_matchesReportSignConventions() {
        ...
        try {
            ...
            Account cash = ensureAccount(company, "CASH-OPEN-TB-" + System.nanoTime(), "Cash", AccountType.ASSET);
            Account revenue = ensureAccount(company, "REV-OPEN-TB-" + System.nanoTime(), "Revenue", AccountType.REVENUE);
            ...
            TemporalBalanceService.TrialBalanceSnapshot temporal = temporalBalanceService.getTrialBalanceAsOf(asOfDate);
            TrialBalanceDto report = reportService.trialBalance(asOfDate);

            TemporalBalanceService.TrialBalanceEntry temporalRevenue = temporal.entries().stream()
                    .filter(entry -> revenue.getId().equals(entry.accountId()))
                    .findFirst()
                    .orElseThrow();
            TrialBalanceDto.Row reportRevenue = report.rows().stream()
