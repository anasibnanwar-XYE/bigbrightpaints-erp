...
class CR_DispatchOrderLookupReadOnlyIT extends AbstractIntegrationTest {
    ...
    @BeforeEach
    void seedUser() {
        dataSeeder.ensureUser(USER_EMAIL, USER_PASSWORD, "Dispatch Lookup", COMPANY_CODE,
    ...
    void getByOrder_withoutSlip_isReadOnlyAndFailsClosed() {
        SalesOrder order = dataSeeder.ensureSalesOrder(COMPANY_CODE, "SO-" + shortId(), new BigDecimal("100.00"));
        ...
        assertThat(packagingSlipRepository.findAllByCompanyAndSalesOrderId(company, order.getId())).isEmpty();
        assertThat(inventoryMovementRepository.findByFinishedGood_CompanyAndReferenceTypeAndReferenceIdOrderByCreatedAtAsc(
                company, InventoryReference.SALES_ORDER, order.getId().toString())).isEmpty();
        assertThat(journalEntryRepository.findByCompanyAndReferenceNumber(
                company, SalesOrderReference.invoiceReference(order.getOrderNumber()))).isEmpty();
    }
