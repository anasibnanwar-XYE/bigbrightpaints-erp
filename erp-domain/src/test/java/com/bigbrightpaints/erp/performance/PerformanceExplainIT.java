...
class PerformanceExplainIT extends AbstractIntegrationTest {
    ...
    void explainPlansForHotQueries() {
        ...
        Account receivable = accountRepository.findByCompanyAndCodeIgnoreCase(company, "AR")
        ...
        logPlan("sales-orders", explain(
                "EXPLAIN SELECT * FROM sales_orders WHERE company_id = ? ORDER BY created_at DESC LIMIT 25",
                company.getId()));
        logPlan("sales-orders-status", explain(
                "EXPLAIN SELECT * FROM sales_orders WHERE company_id = ? AND status = ? ORDER BY created_at DESC LIMIT 25",
                company.getId(), "BOOKED"));
        logPlan("invoices-company", explain(
                "EXPLAIN SELECT * FROM invoices WHERE company_id = ? ORDER BY issue_date DESC LIMIT 25",
                company.getId()));
        logPlan("invoices-dealer", explain(
                "EXPLAIN SELECT * FROM invoices WHERE company_id = ? AND dealer_id = ? ORDER BY issue_date DESC LIMIT 25",
                company.getId(), dealer.getId()));
        logPlan("journal-lines", explain(
                ...
                company.getId(), receivable.getId(), LocalDate.now().minusDays(30), LocalDate.now()));
        logPlan("outbox-pending", explain(
                "EXPLAIN SELECT * FROM orchestrator_outbox "
                        + "WHERE status = 'PENDING' AND dead_letter = false "
                        + "AND next_attempt_at <= now() "
                        + "ORDER BY created_at ASC LIMIT 10"));
        logPlan("inventory-movements", explain(
                "EXPLAIN SELECT * FROM inventory_movements "
                        + "WHERE reference_type = ? AND reference_id = ? "
                        + "ORDER BY created_at ASC",
                "DISPATCH", "PERF-REF"));
        logPlan("dealer-ledger", explain(
