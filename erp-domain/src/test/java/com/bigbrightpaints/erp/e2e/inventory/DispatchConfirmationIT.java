...
class DispatchConfirmationIT extends AbstractIntegrationTest {
    ...
    @BeforeEach
    void setup() {
        dataSeeder.ensureUser("dispatch@bbp.com", "dispatch123", "Dispatch User", COMPANY_CODE,
    ...
    void confirmDispatch_consumesStock_andSetsSlipToDispatched() {
        ...
        finishedGoodsService.registerBatch(new FinishedGoodBatchRequest(
                fg.getId(), "BATCH-DISP", new BigDecimal("50"), new BigDecimal("10.00"), Instant.now(), null));
        ...
        SalesOrderRequest orderReq = new SalesOrderRequest(
                ...
                "INR",
                null,
                List.of(new SalesOrderItemRequest(fg.getProductCode(), "Dispatch Item", new BigDecimal("10"), new BigDecimal("10.00"), BigDecimal.ZERO)),
                "EXCLUSIVE",
                null,
                null,
                null
    ...
    private void ensureCatalogProduct(FinishedGood fg) {
        ProductionBrand brand = productionBrandRepository.findByCompanyAndCodeIgnoreCase(fg.getCompany(), "DISP-BRAND")
                .orElseGet(() -> {
