...
class TS_GstRoundingDeterminismContractTest {
  ...
  void salesTaxAndDiscountMathUsesDeterministicHalfUpRounding() {
    TruthSuiteFileAssert.assertContains(
        ...
        "BigDecimal divisor = BigDecimal.ONE.add(rate.divide(new BigDecimal(\"100\"), 6,"
  ...
  void gstDecisioningAndTaxReturnFallbackAreFailClosed() {
    TruthSuiteFileAssert.assertContains(
        ...
        "throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(\"Unknown GST"
            + " treatment \" + value);");
    ...
    TruthSuiteFileAssert.assertContains(
        ...
        "throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(\"GST rate must"
            + " be zero or positive\");",
    ...
    TruthSuiteFileAssert.assertContains(
        ...
        "BigDecimal outputTaxBalance = sumTax(company, taxConfig.outputTaxAccountId(), start, end,"
            + " true);",
        "BigDecimal inputTaxBalance = sumTax(company, taxConfig.inputTaxAccountId(), start, end,"
            + " false);",
