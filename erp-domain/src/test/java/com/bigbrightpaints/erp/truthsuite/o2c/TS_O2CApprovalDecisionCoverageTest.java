...
class TS_O2CApprovalDecisionCoverageTest {
  ...
  void isOverrideApprovedFailsClosedWhenMakerCheckerBoundaryDiffersOnlyByCaseAndWhitespace() {
    CreditLimitOverrideRequest request =
        approvedRequest(new BigDecimal("80.00"), new BigDecimal("15.00"));
  ...
  void isOverrideApprovedFailsClosedForDealerSlipAndOrderMismatches() {
    ...
    assertThat(
            service.isOverrideApproved(
                3002L,
                company,
                dealer(99L, new BigDecimal("150.00")),
                approvedSlip,
                approvedOrder,
                null))
        .isFalse();
    assertThat(
            service.isOverrideApproved(
                3002L, company, approvedDealer, slip(99L), approvedOrder, null))
        .isFalse();
    assertThat(
            service.isOverrideApproved(
                3002L, company, approvedDealer, approvedSlip, order(99L), null))
        .isFalse();
  }
  ...
  void isOverrideApprovedFailsClosedWhenDispatchHeadroomCannotBeEvaluated() {
    CreditLimitOverrideRequest request =
        approvedRequest(new BigDecimal("90.00"), new BigDecimal("12.00"));

    when(creditLimitOverrideRequestRepository.findByCompanyAndId(company, 3003L))
        .thenReturn(Optional.of(request));

    boolean approved =
        service.isOverrideApproved(3003L, company, null, null, null, new BigDecimal("90.00"));
  ...
  void isOverrideApprovedAllowsDeterministicTolerancePathWhenApprovalContextMatches() {
    ...
    boolean approved =
        service.isOverrideApproved(
            3004L, company, approvedDealer, approvedSlip, approvedOrder, new BigDecimal("80.01"));

    assertThat(approved).isTrue();
  }

  private CreditLimitOverrideRequest approvedRequest(
      BigDecimal dispatchAmount, BigDecimal requiredHeadroom) {
