...
class TS_P2PPurchaseAuditTrailRepositoryCompatibilityTest {
  ...
  void purchasingRepositorySupportsAuditTrailJournalLookups() {
    TruthSuiteFileAssert.assertContains(
        ...
        "Optional<RawMaterialPurchase> findByCompanyAndJournalEntry(Company company, JournalEntry"
            + " journalEntry);",
        "List<RawMaterialPurchase> findByCompanyAndJournalEntry_IdIn(Company company, List<Long>"
            + " journalEntryIds);");
  }
  ...
  void auditTrailJournalLookupsKeepDocumentLinkEntityGraph() {
    TruthSuiteFileAssert.assertContainsInOrder(
        RAW_MATERIAL_PURCHASE_REPOSITORY,
        "@EntityGraph(attributePaths = {\"supplier\", \"journalEntry\", \"purchaseOrder\","
            + " \"goodsReceipt\"})",
        "Optional<RawMaterialPurchase> findByCompanyAndJournalEntry(Company company, JournalEntry"
            + " journalEntry);",
        "@EntityGraph(attributePaths = {\"supplier\", \"journalEntry\", \"purchaseOrder\","
