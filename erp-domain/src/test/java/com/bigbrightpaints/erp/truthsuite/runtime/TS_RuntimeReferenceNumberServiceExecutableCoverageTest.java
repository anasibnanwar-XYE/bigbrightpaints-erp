...
class TS_RuntimeReferenceNumberServiceExecutableCoverageTest {
  ...
  void reference_generation_is_deterministic_with_timezone_fallback_and_metadata_audit() {
    ...
    when(companyClock.today(eq(invalidTimezoneCompany)))
    ...
    assertThat(service.dealerReceiptReference(invalidTimezoneCompany, dealer))
        .startsWith("RCPT-DEALER-001-");
    assertThat(service.salesOrderReference(invalidTimezoneCompany, "SO/2026/001"))
        .startsWith("SALE-SO2026001-");
    assertThat(service.supplierPaymentReference(invalidTimezoneCompany, supplier))
        .startsWith("SUP-SUPPLIER-001-");
    assertThat(service.payrollPaymentReference(invalidTimezoneCompany)).startsWith("PAYROLL-");
    assertThat(service.rawMaterialReceiptReference(invalidTimezoneCompany, "rm:batch/1"))
    ...
    assertThat(service.purchaseReference(invalidTimezoneCompany, supplier, "INV-001"))
        .startsWith("RMP-");
    assertThat(service.purchaseReturnReference(invalidTimezoneCompany, supplier))
        .startsWith("PRN-TRUTH-");
    assertThat(service.inventoryAdjustmentReference(invalidTimezoneCompany, "damaged-return"))
        .startsWith("ADJ-TRUTH-");
    assertThat(service.openingStockReference(invalidTimezoneCompany))
        .startsWith("OPEN-STOCK-TRUTH-");
  ...
  void purchase_reference_key_is_compacted_and_hashed_to_max_length() {
    ReferenceNumberService service =
        new ReferenceNumberService(
            mock(NumberSequenceService.class), mock(AuditService.class), mock(CompanyClock.class));

    Company longCompany =
        company(20L, "COMPANY-CODE-WITH-VERY-LONG-TOKEN-TO-FORCE-COMPACTION-1234567890", "UTC");
  ...
  void purchase_key_compaction_covers_small_max_lengths_and_nullable_entities() {
    ...
    String sevenKey =
        service.purchaseReferenceKey(
            maxSevenCompany, maxSevenSupplier, "INVOICE-TOKEN-FOR-SEVEN-MAX");
    ...
    String eightKey =
        service.purchaseReferenceKey(
            maxEightCompany, maxEightSupplier, "INVOICE-TOKEN-FOR-EIGHT-MAX");
