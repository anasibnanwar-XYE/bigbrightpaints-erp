...
public class SmtpPropertiesValidator {
  ...
  void validateSmtp() {
    ...
    if (!smtpAuthEnabled) {
      log.info(
          "SMTP credential validation skipped because spring.mail.properties.mail.smtp.auth=false");
      return;
    }
    ...
    if ("changeme".equalsIgnoreCase(smtpPassword)) {
      throw new IllegalStateException(
          "SMTP password uses default 'changeme'; set a real credential for production");
    }
