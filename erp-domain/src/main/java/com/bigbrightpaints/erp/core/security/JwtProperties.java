...
public class JwtProperties implements EnvironmentAware {
  ...
  private static final Set<String> UNSAFE_STATIC_SECRETS =
      Set.of(
          "changeme",
  ...
  void validate() {
    ...
    if (!StringUtils.hasText(secret)) {
      if (isTestOnlyRuntime()) {
        ...
        log.warn(
            "JWT secret missing in test-only profile context {}; generated ephemeral in-memory"
                + " secret",
            activeProfilesForLogging());
        return;
      }
      throw new IllegalStateException(
          "JWT secret must be provided via configuration/environment for non-test runtime"
              + " profiles");
    }
    ...
    if (isUnsafeStaticSecret(secret)) {
      if (isTestOnlyRuntime()) {
        log.warn(
            "JWT secret is using a known static placeholder in test-only profile context {};"
                + " override with env secret for non-test runtime",
            activeProfilesForLogging());
        return;
      }
      throw new IllegalStateException(
          "JWT secret uses an unsafe static placeholder; provide a strong random value via"
  ...
  private boolean isUnsafeStaticSecret(String candidate) {
    ...
    if (normalized.contains("changeme")
        || normalized.contains("please-override")
