...
public class BbpSampleDataInitializer {
  ...
  @Bean
  CommandLineRunner seedBbpSamples(
    ...
    return args -> {
      ...
      ProductionBrand brand =
          brandRepository
              .findByCompanyAndCodeIgnoreCase(company, "BBP")
              .orElseGet(
                  () -> {
      ...
      RawMaterial solvent =
          ensureRawMaterial(
              company, rawMaterialRepository, accounts.get("INV"), "RM-SOLV", "Solvent Mix", "L");
      RawMaterial can =
          ensureRawMaterial(
              company,
              rawMaterialRepository,
  ...
  private Map<String, Account> loadAccounts(AccountRepository repo, Company company) {
    ...
    repo.findByCompanyOrderByCodeAsc(company)
        .forEach(acc -> map.put(acc.getCode().toUpperCase(), acc));
    return map;
  }
  ...
  private void seedBatch(
      RawMaterialBatchRepository batchRepo,
  ...
  private void ensureFinishedGood(
      ProductionProductRepository productRepo,
    ...
    ProductionProduct product =
        productRepo
            .findByCompanyAndSkuCode(brand.getCompany(), sku)
            .orElseGet(
                () -> {
    ...
    fgRepo
        .findByCompanyAndProductCode(brand.getCompany(), sku)
        .orElseGet(
            () -> {
