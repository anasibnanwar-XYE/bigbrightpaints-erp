...
public class LicensingGuard implements ApplicationRunner {
  ...
  public void run(ApplicationArguments args) {
    if (!properties.isEnforce()) {
      log.info(
          "CryptoLens licensing enforcement disabled (erp.licensing.enforce=false). Set"
              + " ERP_LICENSE_ENFORCE=true in secured environments.");
      return;
    }

    if (properties.getProductId() <= 0) {
      throw new IllegalStateException(
          "CryptoLens product id must be configured (ERP_LICENSE_PRODUCT_ID).");
    }
    if (!properties.hasLicenseKey()) {
      throw new IllegalStateException(
          "CryptoLens license key missing. Set ERP_LICENSE_KEY before starting the service.");
    }

    log.info(
        "CryptoLens license configured for product {} ({}), algorithm {}, created {}.",
        properties.getProductId(),
        properties.getDescription(),
        properties.getAlgorithm(),
        properties.getCreated());
    log.info(
        "License key supplied ({} characters); masked={}",
        properties.getLicenseKey().length(),
        mask(properties.getLicenseKey()));

    if (!StringUtils.hasText(properties.getAccessToken())) {
      log.warn(
          "No CryptoLens access token configured (ERP_LICENSE_ACCESS_TOKEN). Remote activation"
              + " checks are skipped.");
    }
  }
