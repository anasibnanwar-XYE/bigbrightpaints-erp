...
public final class LegacyDispatchInvoiceLinkMatcher {
  ...
  public static boolean isSlipLinkedToInvoice(
      PackagingSlip slip,
      Invoice invoice,
    ...
    boolean hasExplicitInvoiceLinks =
        candidateSlips != null
            && candidateSlips.stream()
                .anyMatch(candidate -> candidate != null && candidate.getInvoiceId() != null);
    ...
    long candidateCount =
        candidateSlips == null ? 0 : candidateSlips.stream().filter(Objects::nonNull).count();
