...
public final class ValidationUtils {
  ...
  public static <E extends Enum<E>> E parseEnum(
      Class<E> enumType, String rawValue, String fieldName) {
    String normalized = requireNotBlank(rawValue, fieldName).toUpperCase();
    try {
      return Enum.valueOf(enumType, normalized);
    } catch (IllegalArgumentException ex) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_INVALID_INPUT, "Invalid " + fieldName + ": " + rawValue, ex);
    }
  }
  ...
  public static <T> T requireEntity(Optional<T> entity, String entityName, Object identifier) {
    if (entity == null || entity.isEmpty()) {
      throw new ApplicationException(
          ErrorCode.BUSINESS_ENTITY_NOT_FOUND, entityName + " not found: " + identifier);
    }
    return entity.get();
  }
