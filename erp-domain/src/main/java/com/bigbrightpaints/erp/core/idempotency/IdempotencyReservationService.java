...
public class IdempotencyReservationService {
  ...
  public String requireKey(String raw, String label) {
    ...
    if (!StringUtils.hasText(key)) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_MISSING_REQUIRED_FIELD, "Idempotency key is required for " + label);
    }
  ...
  public <T> void assertAndRepairSignature(
      T record,
  ...
  public <T> Reservation<T> reserve(
      Supplier<Optional<T>> existingLookup, Supplier<T> reservationCreator) {
