...
public interface OutboxEventRepository extends JpaRepository<OutboxEvent, UUID> {
  ...
  long countByStatusAndDeadLetterFalseAndRetryCountGreaterThan(
      OutboxEvent.Status status, int retryCount);

  long countByStatusAndDeadLetterFalseAndNextAttemptAtLessThanEqual(
      OutboxEvent.Status status, Instant nextAttemptAt);

  long countByStatusAndDeadLetterFalseAndLastErrorStartingWith(
      OutboxEvent.Status status, String lastErrorPrefix);
  ...
  @Query(
      ...
      """)
  long countAmbiguousPublishingEvents(
