...
public class OrchestratorIdempotencyService {
  ...
  private static final String ROLLBACK_FAILURE_MESSAGE =
      "Outer transaction rolled back before commit";
  ...
  public OrchestratorIdempotencyService(
      OrchestratorCommandRepository commandRepository,
        ...
        status -> {
          ...
          boolean reserved =
              commandRepository.reserveScope(
                      company.getId(), commandName, key, requestHash, traceId)
                  > 0;

          OrchestratorCommand existing =
              commandRepository
                  .lockByScope(company.getId(), commandName, key)
                  .orElseThrow(
                      () ->
                          new IllegalStateException(
                              "Idempotency reservation exists but command row not found"));
          ...
          new TransactionSynchronization() {
            ...
            public void afterCompletion(int status) {
              if (status == TransactionSynchronization.STATUS_ROLLED_BACK) {
                safelyPersistStatus(
                    commandId, managed -> managed.markFailed(ROLLBACK_FAILURE_MESSAGE), "FAILED");
              }
            }
          });
      return;
  ...
  private void persistStatusUpdate(
      UUID commandId, java.util.function.Consumer<OrchestratorCommand> mutator) {
          ...
          OrchestratorCommand managed =
              commandRepository
                  ...
                  .orElseThrow(
                      () ->
                          new IllegalStateException(
                              "Orchestrator command row not found for id " + commandId));
  ...
  private void safelyPersistStatus(
      UUID commandId,
    ...
    try {
      ...
      log.error(
          "Failed to persist orchestrator command {} transition to {}",
          commandId,
          targetStatus,
          ex);
    }
  ...
  private String serializeRequestPayload(String commandName, Object requestPayload) {
    try {
      ...
      throw new IllegalStateException(
          "Unable to deterministically hash orchestrator payload for command " + commandName, ex);
    }
  }
}
