...
public class FactoryService {
  ...
  public List<ProductionBatchDto> listBatches() {
    Company company = companyContextService.requireCurrentCompany();
    return batchRepository.findByCompanyOrderByProducedAtDesc(company).stream()
        .map(this::toDto)
        .toList();
  }
  ...
  private ProductionBatchDto logBatchInternal(
    ...
    batch.setBatchNumber(
        normalizedBatchNumber != null ? normalizedBatchNumber : request.batchNumber());
  ...
  private void registerFinishedGoodsBatch(ProductionPlan plan, ProductionBatch batch) {
    FinishedGood finishedGood =
        finishedGoodsService.lockFinishedGoodByProductCode(plan.getProductName());
  ...
  private ProductionBatchDto createBatchWithNaturalKey(
      Company company, Long planId, ProductionBatchRequest request, String normalizedBatchNumber) {
    ProductionPlan plan =
        planId != null ? companyEntityLookup.requireProductionPlan(company, planId) : null;
    Instant producedAt = CompanyTime.now(company);
    int inserted =
        batchRepository.insertIfAbsent(
            company.getId(),
  ...
  private ProductionBatchDto resolveExistingBatch(
  ...
  private boolean isBatchPayloadEquivalent(
      ProductionBatch existing,
      Long planId,
      ProductionBatchRequest request,
      String normalizedBatchNumber) {
