...
public class PackingIdempotencyService {
  ...
  public PackingIdempotencyService(
  ...
  public String packingRequestHash(PackingRequest request, LocalDate packedDate) {
    ...
    payload
        .append("log=")
        .append(request.productionLogId())
        .append("|date=")
        .append(packedDate)
        .append("|by=")
    ...
    if (request.lines() != null) {
      ...
      for (var line : request.lines()) {
        ...
        payload
            ...
            .append(':')
            .append(line.childFinishedGoodId())
            .append(':')
            .append(line.childBatchCount())
            .append(':')
            .append(decimalToken(line.quantityLiters()))
            .append(':')
            .append(line.piecesCount())
            .append(':')
            .append(line.boxesCount())
            .append(':')
            .append(line.piecesPerBox());
      }
    }
    return IdempotencyUtils.sha256Hex(payload.toString());
  }

  public IdempotencyReservation reserveIdempotencyRecord(
      Company company, Long productionLogId, String idempotencyKey, String idempotencyHash) {
    Optional<PackingRequestRecord> existing =
        packingRequestRecordRepository.findByCompanyAndIdempotencyKey(company, idempotencyKey);
  ...
  private IdempotencyReservation resolveExistingIdempotency(
      PackingRequestRecord record, Long productionLogId, String idempotencyHash) {
    ...
    if (StringUtils.hasText(record.getIdempotencyHash())
      ...
      throw new ApplicationException(
              ErrorCode.CONCURRENCY_CONFLICT, "Idempotency payload mismatch for packing request")
          .withDetail("idempotencyKey", record.getIdempotencyKey())
  ...
  public record IdempotencyReservation(
      PackingRequestRecord record, ProductionLogDetailDto replayResult) {}
}
