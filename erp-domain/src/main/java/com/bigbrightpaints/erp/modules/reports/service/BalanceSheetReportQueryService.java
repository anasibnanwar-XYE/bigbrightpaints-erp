...
public class BalanceSheetReportQueryService {
  ...
  public BalanceSheetReportQueryService(
  ...
  public BalanceSheetDto generate(FinancialReportQueryRequest request) {
    ...
    if (comparison != null) {
      ...
      comparative =
          new BalanceSheetDto.Comparative(
  ...
  private BalanceSheetSnapshot summarize(ReportQuerySupport.FinancialQueryWindow window) {
    ...
    for (BalanceLine line : lines) {
      BalanceSheetDto.SectionLine dto =
          new BalanceSheetDto.SectionLine(
              line.accountId(), line.code(), line.name(), line.amount());
    ...
    if (!usesClosedSnapshot(window)) {
      ...
      if (currentEarnings.compareTo(BigDecimal.ZERO) != 0) {
        ...
        equityLines.add(
            new BalanceSheetDto.SectionLine(
                null, CURRENT_EARNINGS_CODE, CURRENT_EARNINGS_NAME, currentEarnings));
      }
    }

    boolean balanced =
        totalAssets.subtract(totalLiabilities.add(totalEquity)).abs().compareTo(BALANCE_TOLERANCE)
            <= 0;
  ...
  private record BalanceLine(
      Long accountId, String code, String name, AccountType type, BigDecimal amount) {}
