...
public class BulkPackingOrchestrator {
  ...
  public BulkPackingOrchestrator(
  ...
  public void validatePackLines(BulkPackRequest request) {
    if (request == null) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_INVALID_INPUT, "Bulk pack request is required");
    }
    if (request.packs() == null || request.packs().isEmpty()) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_INVALID_INPUT, "At least one pack line is required");
    }
    ...
    for (BulkPackRequest.PackLine line : request.packs()) {
      if (line == null || line.childSkuId() == null) {
        throw new ApplicationException(
            ErrorCode.VALIDATION_INVALID_INPUT, "Child SKU is required for each pack line");
      }
  ...
  public FinishedGoodBatch createChildBatch(
    ...
    try {
      ...
      throw new ApplicationException(
          ErrorCode.BUSINESS_ENTITY_NOT_FOUND, "Child SKU not found: " + line.childSkuId(), ex);
    }

    BigDecimal sizeInLiters = extractSizeInLiters(line.sizeLabel(), line.unit());
    BigDecimal childUnitCost =
        bulkUnitCost.multiply(sizeInLiters).add(packagingCostPerUnit).setScale(4, COST_ROUNDING);

    FinishedGoodBatchRegistrar.ReceiptRegistrationResult result =
  ...
  private JournalEntryRequest.JournalLineRequest toChildDebitLine(FinishedGoodBatch childBatch) {
    ...
    if (childAccountId == null) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_INVALID_REFERENCE,
          "Child FG "
              + childBatch.getFinishedGood().getProductCode()
              + " missing valuation account");
    }
    BigDecimal childValue =
        childBatch.getUnitCost().multiply(childBatch.getQuantityTotal()).setScale(2, COST_ROUNDING);
