...
public record BulkPackRequest(
    @NotNull(message = "Bulk batch ID is required") Long bulkBatchId,
    @NotEmpty(message = "At least one pack line is required") @Valid List<PackLine> packs,
    LocalDate packDate,
    String packedBy,
    String notes,
    /**
  ...
  public record PackLine(
      @NotNull(message = "Child SKU ID is required") Long childSkuId,
