...
public class PackingJournalBuilder {
  ...
  public List<JournalEntryRequest.JournalLineRequest> buildWipPackagingConsumptionLines(
      Long wipAccountId,
      String memo,
  ...
  public List<JournalEntryRequest.JournalLineRequest> buildBulkToSizePackingLines(
      Long bulkAccountId,
    ...
    packagingCredits.forEach(
        (accountId, amount) -> {
          BigDecimal roundedAmount = roundedMoney(amount);
          if (roundedAmount.compareTo(BigDecimal.ZERO) > 0) {
            lines.add(
                line(accountId, "Packaging material consumption", BigDecimal.ZERO, roundedAmount));
          }
        });
  ...
  private List<JournalEntryRequest.JournalLineRequest> rebalance(
      List<JournalEntryRequest.JournalLineRequest> lines, boolean adjustFirstDebit) {
    ...
    lines.set(
        0,
        line(first.accountId(), first.description(), first.debit().add(variance), first.credit()));
    return lines;
  }
  ...
  private JournalEntryRequest.JournalLineRequest line(
      Long accountId, String description, BigDecimal debit, BigDecimal credit) {
    return new JournalEntryRequest.JournalLineRequest(
        accountId, description, roundedMoney(debit), roundedMoney(credit));
  }
}
