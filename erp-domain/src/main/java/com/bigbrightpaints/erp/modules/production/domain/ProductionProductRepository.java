...
public interface ProductionProductRepository
    extends JpaRepository<ProductionProduct, Long>, JpaSpecificationExecutor<ProductionProduct> {
  ...
  Optional<ProductionProduct> findByBrandAndProductNameIgnoreCase(
      ProductionBrand brand, String productName);

  Optional<ProductionProduct> findTopByCompanyAndSkuCodeStartingWithOrderBySkuCodeDesc(
  ...
  @EntityGraph(attributePaths = "brand")
  List<ProductionProduct> findByCompanyAndVariantGroupIdOrderByProductNameAsc(
      Company company, UUID variantGroupId);
  ...
  @Query(
      "select p from ProductionProduct p where p.brand in :brands and lower(p.productName) in"
          + " :names")
  List<ProductionProduct> findByBrandInAndProductNameInIgnoreCase(
      @Param("brands") Collection<ProductionBrand> brands,
