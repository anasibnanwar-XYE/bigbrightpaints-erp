...
public final class DispatchMetadataValidator {
  ...
  public static boolean shouldEnforceValidation(
      DispatchConfirmRequest request, BooleanSupplier dispatchedSlipReplaySupplier) {
  ...
  public static void validate(DispatchConfirmRequest request) {
    if (!hasRequiredMetadata(request)) {
      boolean hasTransportActor =
          StringUtils.hasText(request.transporterName())
              || StringUtils.hasText(request.driverName());
      if (!hasTransportActor) {
        throw ValidationUtils.invalidInput(
            "Dispatch confirmation requires transporterName or driverName");
      }
