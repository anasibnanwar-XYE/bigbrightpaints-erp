...
public class PayrollCalculationService {
  ...
  public PayrollRunDto calculatePayroll(Long payrollRunId) {
    ...
    if (run.getStatus() != PayrollRun.PayrollStatus.DRAFT) {
      throw new ApplicationException(
              ErrorCode.BUSINESS_INVALID_STATE, "Can only calculate payroll in DRAFT status")
    ...
    List<Employee> employees =
        run.getRunType() == PayrollRun.RunType.WEEKLY
  ...
  public WeeklyPaySummaryDto getWeeklyPaySummary(LocalDate weekEndingDate) {
    ...
    List<Employee> labourers =
        employeeRepository.findByCompanyAndEmployeeTypeAndStatus(
            company, Employee.EmployeeType.LABOUR, "ACTIVE");
    List<Attendance> attendance =
        attendanceRepository.findByEmployeeTypeAndStatusAndDateRange(
    ...
    for (Employee employee : labourers) {
      BigDecimal presentDays =
          presentDaysByEmployee.getOrDefault(employee.getId(), BigDecimal.ZERO);
      BigDecimal otHours = overtimeHoursByEmployee.getOrDefault(employee.getId(), BigDecimal.ZERO);
      BigDecimal doubleOtHours =
          doubleOtHoursByEmployee.getOrDefault(employee.getId(), BigDecimal.ZERO);
      BigDecimal holidayDays =
          holidayDaysByEmployee.getOrDefault(employee.getId(), BigDecimal.ZERO);
      ...
      BigDecimal standardHoursPerDay =
          payrollCalculationSupport.requireValidStandardHoursPerDay(employee);
      BigDecimal hourlyRate =
          employee.getDailyRate().divide(standardHoursPerDay, 2, RoundingMode.HALF_UP);
      BigDecimal otPay =
          hourlyRate.multiply(employee.getOvertimeRateMultiplier()).multiply(otHours);
      BigDecimal doubleOtPay =
          hourlyRate.multiply(employee.getDoubleOtRateMultiplier()).multiply(doubleOtHours);
      BigDecimal grossPay = basePay.add(otPay).add(doubleOtPay);
      BigDecimal basicComponentForSummary =
          employee.getSalaryStructureTemplate() != null
              ? nonNull(employee.getSalaryStructureTemplate().getBasicPay())
              : basePay;
      BigDecimal pfDeduction =
          statutoryDeductionEngine.calculatePfDeduction(basicComponentForSummary, employee);
      ...
      BigDecimal loanDeduction =
          payrollCalculationSupport.calculateLoanDeduction(grossPay, employee);
      BigDecimal totalDeductionsForSummary =
          pfDeduction.add(esiDeduction).add(professionalTaxDeduction).add(loanDeduction);
    ...
    return new WeeklyPaySummaryDto(
        weekStart, weekEnd, labourers.size(), totalBasePay, totalOtPay, totalNetPay, employeePay);
  }
  ...
  public MonthlyPaySummaryDto getMonthlyPaySummary(int year, int month) {
    ...
    for (Employee employee : staff) {
      ...
      if (template != null) {
        ...
        BigDecimal specialComponent =
            money(nonNull(template.getSpecialAllowance()).multiply(payableRatio));
      ...
      BigDecimal pfDeduction =
          statutoryDeductionEngine.calculatePfDeduction(basicComponentForPf, employee);
      BigDecimal esiDeduction = statutoryDeductionEngine.calculateEsiDeduction(grossPay, employee);
      BigDecimal professionalTaxDeduction =
          statutoryDeductionEngine.calculateProfessionalTaxDeduction(employee, null);
      BigDecimal loanDeduction =
          payrollCalculationSupport.calculateLoanDeduction(grossPay, employee);
      BigDecimal totalDed =
          money(pfDeduction.add(esiDeduction).add(professionalTaxDeduction).add(loanDeduction));
      BigDecimal netPay = money(grossPay.subtract(totalDed));
      if (netPay.compareTo(BigDecimal.ZERO) < 0) {
        netPay = BigDecimal.ZERO;
      }
      employeePay.add(
          new EmployeeMonthlyPayDto(
              employee.getId(),
              employee.getFullName(),
    ...
    return new MonthlyPaySummaryDto(
        year, month, staff.size(), totalGross, totalDeductions, totalNet, employeePay);
  }
  ...
  private PayrollRunLine calculateEmployeePay(PayrollRun run, Employee employee) {
    ...
    BigDecimal standardHoursPerDay =
        payrollCalculationSupport.requireValidStandardHoursPerDay(employee);
    BigDecimal hourlyRate =
        dailyRate.compareTo(BigDecimal.ZERO) > 0
    ...
    BigDecimal overtimePay =
        money(otRate.multiply(overtimeHours).add(doubleOtRate.multiply(doubleOtHours)));
    ...
    if (template != null && run.getRunType() == PayrollRun.RunType.MONTHLY) {
      BigDecimal workingDays = BigDecimal.valueOf(Math.max(1, employee.getWorkingDaysPerMonth()));
      BigDecimal leaveWithoutPayDays =
          absentDays.add(leaveDays).add(halfDays.multiply(HALF_DAY_FACTOR));
      BigDecimal payableRatio =
          BigDecimal.ONE.subtract(leaveWithoutPayDays.divide(workingDays, 6, RoundingMode.HALF_UP));
      if (payableRatio.compareTo(BigDecimal.ZERO) < 0) {
        payableRatio = BigDecimal.ZERO;
      }
    ...
    BigDecimal pfDeduction =
        statutoryDeductionEngine.calculatePfDeduction(basicComponent, employee);
    BigDecimal esiDeduction = statutoryDeductionEngine.calculateEsiDeduction(grossPay, employee);
    BigDecimal tdsDeduction =
        statutoryDeductionEngine.calculateTdsDeduction(grossPay, run, employee);
    BigDecimal professionalTaxDeduction =
        statutoryDeductionEngine.calculateProfessionalTaxDeduction(employee, run);
