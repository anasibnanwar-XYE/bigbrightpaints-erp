...
public class SuperAdminService {
  ...
  public SuperAdminDashboardDto getDashboard() {
    ...
    long activeTenants =
        tenants.stream().filter(tenant -> STATUS_ACTIVE.equals(tenant.status())).count();
    long suspendedTenants =
        tenants.stream().filter(tenant -> STATUS_SUSPENDED.equals(tenant.status())).count();
    long deactivatedTenants =
        tenants.stream().filter(tenant -> STATUS_DEACTIVATED.equals(tenant.status())).count();
