...
public class PackingLineResolver {
  ...
  public Integer resolvePiecesPerBox(PackingLineRequest line, SizeVariant sizeVariant) {
    ...
    if (sizeVariant != null
        && sizeVariant.getCartonQuantity() != null
        && sizeVariant.getCartonQuantity() > 0) {
      return sizeVariant.getCartonQuantity();
    }
    return null;
  }

  public int resolvePiecesCountForLine(
      PackingLineRequest line, Integer resolvedPiecesPerBox, int lineNumber) {
    if (line.piecesCount() != null && line.piecesCount() > 0) {
      return line.piecesCount();
    }
    if (line.boxesCount() != null
        && line.boxesCount() > 0
        && resolvedPiecesPerBox != null
        && resolvedPiecesPerBox > 0) {
      return line.boxesCount() * resolvedPiecesPerBox;
    }
    throw ValidationUtils.invalidInput(
        "Pieces count or boxes × pieces per box required for line " + lineNumber);
  }
  ...
  private BigDecimal resolveLitersPerUnit(
      SizeVariant sizeVariant, String normalizedSize, int lineNumber) {
    if (sizeVariant != null
  ...
  private SizeVariant createDefaultSizeVariant(
      Company company, ProductionProduct product, String packagingSize) {
