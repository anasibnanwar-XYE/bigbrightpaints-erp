...
public class InventoryBatchQueryService {
  ...
  public InventoryBatchQueryService(
  ...
  public List<InventoryExpiringBatchDto> listExpiringSoonBatches(int days) {
    ...
    List<InventoryExpiringBatchDto> rawMaterialRows =
        rawMaterialBatchRepository.findExpiringSoonByCompany(company, today, cutoff).stream()
            .map(
                batch ->
                    new InventoryExpiringBatchDto(
                        "RAW_MATERIAL",
                        batch.getId(),
                        batch.getPublicId(),
    ...
    List<InventoryExpiringBatchDto> finishedGoodRows =
        finishedGoodBatchRepository.findExpiringSoonByCompany(company, today, cutoff).stream()
            .map(
                batch ->
                    new InventoryExpiringBatchDto(
                        "FINISHED_GOOD",
    ...
    return java.util.stream.Stream.concat(rawMaterialRows.stream(), finishedGoodRows.stream())
        .sorted(
            Comparator.comparing(InventoryExpiringBatchDto::expiryDate)
