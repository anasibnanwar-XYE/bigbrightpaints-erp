...
public class PortalInsightsService {
  private static final Set<String> REVENUE_RECOGNIZED_INVOICE_STATUSES =
      Set.of("ISSUED", "PAID", "PARTIAL");
  ...
  public DashboardInsights dashboard() {
    ...
    BigDecimal recognizedRevenueLast30 =
        sumRecognizedRevenueOnOrAfter(company, recognizedRevenueCutoff);
    List<PackagingSlip> slips = packagingSlipRepository.findByCompanyOrderByCreatedAtDesc(company);
    List<Employee> employees =
        hrPayrollEnabled ? employeeRepository.findByCompanyOrderByFirstNameAsc(company) : List.of();
    ...
    long activeEmployees =
        employees.stream().filter(emp -> "ACTIVE".equalsIgnoreCase(emp.getStatus())).count();
    double fulfilment =
        ratio(
    ...
    highlights.add(
        new DashboardInsights.HighlightMetric(
            "Fulfilment SLA",
            percent(fulfilment),
        ...
        plans.stream()
            .collect(
                Collectors.groupingBy(
                    plan -> plan.getStatus().toUpperCase(Locale.ROOT), Collectors.counting()));

    List<DashboardInsights.PipelineStage> pipelineStages =
    ...
    if (hrPayrollEnabled) {
      long onLeave =
          leaveRequestRepository.findByCompanyOrderByCreatedAtDesc(company).stream()
      ...
      hrPulse =
          List.of(
              new DashboardInsights.HrPulseMetric(
                  "Engagement",
                  percent(ratio(activeEmployees, employees.size())),
                  "Active employees"),
              new DashboardInsights.HrPulseMetric(
                  "Leave utilisation",
                  percent(ratio(onLeave, employees.size())),
                  "Approved leave records"),
              new DashboardInsights.HrPulseMetric(
                  "Payroll readiness",
                  formatNumber(payrollDrafts),
                  "Draft payroll runs awaiting approval"));
    }
  ...
  public OperationsInsights operations() {
    ...
    double logisticsSla =
        ratio(
                slips.stream()
                    .filter(slip -> "DISPATCHED".equalsIgnoreCase(slip.getStatus()))
                    .count(),
    ...
    OperationsInsights.OperationsSummary summary =
        new OperationsInsights.OperationsSummary(
            round(productionVelocity), round(logisticsSla), currency(workingCapital));

    List<OperationsInsights.SupplyAlert> supplyAlerts =
        materials.stream()
    ...
    List<OperationsInsights.AutomationRun> automationRuns =
        tasks.stream()
            .limit(4)
            .map(
                task ->
                    new OperationsInsights.AutomationRun(
                        task.getTitle(),
                        task.getStatus(),
  ...
  public WorkforceInsights workforce() {
    Company company = companyContextService.requireCurrentCompany();
    moduleGatingService.requireEnabled(
    ...
    List<WorkforceInsights.SquadSummary> squads =
        byRole.entrySet().stream()
            .sorted(Map.Entry.<String, Long>comparingByValue().reversed())
            .limit(3)
            .map(
                entry ->
                    new WorkforceInsights.SquadSummary(
                        entry.getKey(),
    ...
    payrollRuns.stream()
        ...
        .forEach(
            run ->
                moments.add(
                    new WorkforceInsights.UpcomingMoment(
                        "Payroll run",
                        run.getRunDate().toString(),
                        run.getNotes() != null ? run.getNotes() : "Pending payroll approval")));
    leaveRequests.stream()
        .filter(
            request -> request.getStartDate() != null && !request.getStartDate().isBefore(today))
        .limit(2)
        .forEach(
            request ->
                moments.add(
    ...
    List<WorkforceInsights.PerformanceLeader> leaders =
        employees.stream()
            .filter(emp -> "ACTIVE".equalsIgnoreCase(emp.getStatus()))
            .sorted(
                Comparator.comparing(
                        Employee::getHiredDate, Comparator.nullsLast(Comparator.naturalOrder()))
                    .reversed())
            .limit(3)
            .map(
                emp ->
                    new WorkforceInsights.PerformanceLeader(
                        emp.getFirstName() + " " + emp.getLastName(),
                        emp.getRole() != null ? emp.getRole() : "Operator",
                        emp.getStatus()
                            + " · joined "
                            + (emp.getHiredDate() != null ? emp.getHiredDate() : "recently")))
            .toList();
  ...
  private OperationsInsights.SupplyAlert toSupplyAlert(RawMaterial material) {
    ...
    String detail =
        "Stock "
            ...
            + " · reorder "
            + reorder.stripTrailingZeros().toPlainString();
  ...
  private BigDecimal sumRecognizedRevenue(Company company) {
    return zeroIfNull(
        entityManager
            .createQuery(RECOGNIZED_REVENUE_SUM_QUERY, BigDecimal.class)
  ...
  private BigDecimal sumRecognizedRevenueOnOrAfter(Company company, LocalDate fromDate) {
    return zeroIfNull(
        entityManager
            .createQuery(RECOGNIZED_REVENUE_SUM_SINCE_QUERY, BigDecimal.class)
            .setParameter("company", company)
            .setParameter("statuses", REVENUE_RECOGNIZED_INVOICE_STATUSES)
