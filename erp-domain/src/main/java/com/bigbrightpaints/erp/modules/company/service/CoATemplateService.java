...
public class CoATemplateService {
    ...
    public List<CoATemplateDto> listActiveTemplates() {
        return coATemplateRepository.findByActiveTrueOrderByNameAsc().stream()
                .map(template -> new CoATemplateDto(
                        template.getCode(),
                        template.getName(),
    ...
    public CoATemplate requireActiveTemplate(String code) {
        if (code == null || code.isBlank()) {
            throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput("coaTemplateCode is required");
        }
        return coATemplateRepository.findByCodeIgnoreCaseAndActiveTrue(code.trim())
