...
public interface DealerLedgerRepository extends JpaRepository<DealerLedgerEntry, Long> {
  ...
  @Query(
      "select new com.bigbrightpaints.erp.modules.accounting.dto.DealerBalanceView(e.dealer.id,"
          ...
          + " by e.dealer.id")
  List<DealerBalanceView> aggregateBalancesBetween(
  ...
  List<DealerLedgerEntry> findByCompanyAndDealerAndEntryDateBetweenOrderByEntryDateAscIdAsc(
      Company company, Dealer dealer, java.time.LocalDate start, java.time.LocalDate end);

  List<DealerLedgerEntry> findByCompanyAndDealerAndEntryDateBetweenOrderByEntryDateAsc(
      Company company, Dealer dealer, java.time.LocalDate start, java.time.LocalDate end);

  List<DealerLedgerEntry> findByCompanyAndDealerAndEntryDateBeforeOrderByEntryDateAsc(
      Company company, Dealer dealer, java.time.LocalDate before);

  List<DealerLedgerEntry> findByCompanyAndDealerAndEntryDateLessThanEqualOrderByEntryDateAscIdAsc(
      Company company, Dealer dealer, java.time.LocalDate asOf);

  java.util.Optional<DealerLedgerEntry> findFirstByCompanyAndDealerOrderByEntryDateDescIdDesc(
      Company company, Dealer dealer);

  // Aging report queries
  @Query(
      "SELECT e FROM DealerLedgerEntry e WHERE e.company = :company AND e.dealer = :dealer AND"
          + " e.paymentStatus NOT IN ('PAID','VOID','REVERSED') AND e.invoiceNumber IS NOT NULL"
          + " ORDER BY e.dueDate ASC")
  List<DealerLedgerEntry> findUnpaidByDealer(
      @Param("company") Company company, @Param("dealer") Dealer dealer);

  @Query(
      "SELECT e FROM DealerLedgerEntry e WHERE e.company = :company AND e.paymentStatus NOT IN"
          + " ('PAID','VOID','REVERSED') AND e.invoiceNumber IS NOT NULL ORDER BY e.dealer.id,"
          + " e.dueDate ASC")
  List<DealerLedgerEntry> findAllUnpaid(@Param("company") Company company);

  @Query(
      "SELECT e FROM DealerLedgerEntry e WHERE e.company = :company "
  ...
  List<DealerLedgerEntry> findAllUnpaidAsOf(
      @Param("company") Company company, @Param("asOfDate") java.time.LocalDate asOfDate);

  @Query(
      "SELECT e FROM DealerLedgerEntry e WHERE e.company = :company AND e.paymentStatus NOT IN"
          + " ('PAID','VOID','REVERSED') AND e.invoiceNumber IS NOT NULL AND e.dueDate < :asOfDate"
          + " ORDER BY e.dealer.id, e.dueDate ASC")
  ...
  @Query(
      value =
          "SELECT AVG((paid_date - entry_date)::numeric) FROM dealer_ledger_entries WHERE"
              + " company_id = :companyId AND dealer_id = :dealerId AND paid_date IS NOT NULL",
      nativeQuery = true)
