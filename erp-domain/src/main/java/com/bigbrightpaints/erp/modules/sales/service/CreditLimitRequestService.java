...
public class CreditLimitRequestService {
  ...
  public CreditLimitRequestService(
  ...
  @Transactional
  public CreditLimitRequestDto createRequest(
      CreditLimitRequestCreateRequest request, Long requesterUserId, String requesterEmail) {
    Company company = companyContextService.requireCurrentCompany();
  ...
  public CreditLimitRequestDto rejectRequest(Long id, String decisionReason) {
    ...
    creditRequestRepository.save(creditRequest);
    auditDecision(
        AuditEvent.TRANSACTION_REJECTED, creditRequest, normalizedDecisionReason, Map.of());
    return toDto(creditRequest);
  }
  ...
  private Long requireDealerId(Long dealerId) {
    if (dealerId == null) {
      throw new ApplicationException(
              ErrorCode.VALIDATION_MISSING_REQUIRED_FIELD, "Credit limit request requires dealerId")
  ...
  private CreditRequest requireRequest(Long id) {
    ...
    return creditRequestRepository
        ...
        .orElseThrow(
            () ->
                new ApplicationException(
                    ErrorCode.BUSINESS_ENTITY_NOT_FOUND, "Credit limit request not found"));
  }
  ...
  private BigDecimal requireCurrentDealerCreditLimit(Dealer dealer) {
    if (dealer.getCreditLimit() == null || dealer.getCreditLimit().compareTo(BigDecimal.ZERO) < 0) {
      throw new ApplicationException(
              ErrorCode.BUSINESS_INVALID_STATE, "Dealer credit limit is missing or invalid")
  ...
  private String normalizeStatus(String status) {
    ...
    if (!VALID_STATUSES.contains(normalized)) {
      throw new ApplicationException(
              ErrorCode.VALIDATION_INVALID_INPUT, "Unsupported credit limit request status")
