...
public class DeliveryChallanPdfService {
  ...
  public PdfDocument renderDeliveryChallanPdf(Long packagingSlipId) {
    ...
    if (slip.getConfirmedAt() == null) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Delivery challan is available only after dispatch confirmation");
    }
    ...
    List<DeliveryChallanLineView> lines =
        slip.getLines().stream().map(this::toLineView).filter(Objects::nonNull).toList();

    DeliveryChallanView view =
        new DeliveryChallanView(
  ...
  private byte[] renderPdf(String html) {
    try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
      ...
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Failed to render delivery challan PDF", ex);
    }
  }
  ...
  public record DeliveryChallanLineView(
      String productCode,
      String productName,
      String batchCode,
  ...
  public record DeliveryChallanView(
      String companyName,
      String companyCode,
