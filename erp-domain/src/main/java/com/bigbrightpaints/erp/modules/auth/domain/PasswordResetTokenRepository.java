...
public interface PasswordResetTokenRepository extends JpaRepository<PasswordResetToken, Long> {
  ...
  Optional<PasswordResetToken>
      findTopDeliveredByUserAndIdNotOrderByDeliveredAtDescCreatedAtDescIdDesc(
          @Param("user") UserAccount user, @Param("keepId") Long keepId);

  @Modifying
  @Query("update PasswordResetToken t set t.createdAt = :createdAt where t.id = :tokenId")
  int touchCreatedAt(
      @Param("tokenId") Long tokenId, @Param("createdAt") java.time.Instant createdAt);
  ...
  @Query(
      ...
      """)
  int markDeliveredAt(
      @Param("tokenId") Long tokenId, @Param("deliveredAt") java.time.Instant deliveredAt);
}
