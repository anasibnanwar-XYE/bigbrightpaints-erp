...
public class PackagingSlipService {
  ...
  public List<PackagingSlipDto> listPackagingSlips() {
    ...
    return packagingSlipRepository.findByCompanyOrderByCreatedAtDesc(company).stream()
        .map(this::toSlipDto)
        .toList();
  }
  ...
  public PackagingSlipDto getPackagingSlipByOrder(Long salesOrderId) {
    ...
    PackagingSlip slip =
        packagingSlipRepository
            .findByIdAndCompany(selected.getId(), company)
            .orElseThrow(
                () ->
                    com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
                        "Packaging slip not found"));
  ...
  public PackagingSlipDto updateSlipStatus(Long slipId, String newStatus) {
    ...
    if (slip.isBackorder()) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Backorder slips can only be changed via backorder workflows");
    }
    if ("DISPATCHED".equalsIgnoreCase(slip.getStatus())) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Cannot update status of dispatched slip");
    }
    if (!StringUtils.hasText(newStatus)) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
          "Slip status is required");
    }

    String normalized = newStatus.trim().toUpperCase();
    if ("DISPATCHED".equals(normalized)) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Use dispatch confirmation to mark a slip as dispatched");
    }
    if (!List.of("PENDING", "PENDING_PRODUCTION", "RESERVED", "PENDING_STOCK")
        .contains(normalized)) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
          "Unsupported slip status: " + normalized);
    }
    if (!canTransitionStatus(slip.getStatus(), normalized)) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Invalid slip status transition: " + slip.getStatus() + " -> " + normalized);
    }
  ...
  public PackagingSlipDto cancelBackorderSlip(Long slipId, String username, String reason) {
    ...
    if (!slip.isBackorder() || !"BACKORDER".equalsIgnoreCase(slip.getStatus())) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Only BACKORDER slips can be canceled");
    }

    List<InventoryReservation> reservations =
        inventoryReservationRepository.findByFinishedGoodCompanyAndReferenceTypeAndReferenceId(
            company, InventoryReference.SALES_ORDER, slip.getSalesOrder().getId().toString());
    ...
    slip.setDispatchNotes(
        reason != null
            ? reason
            : "Backorder canceled by " + (username != null ? username : "system"));
  ...
  public Long createBackorderSlip(PackagingSlip originalSlip) {
    ...
    Long existingId =
        findOpenBackorderSlipId(
            originalSlip.getCompany(), originalSlip.getSalesOrder().getId(), originalSlip.getId());
    if (existingId != null) {
      return existingId;
    }
    ...
    backorderSlip.setSlipNumber(
        batchNumberService.nextPackagingSlipNumber(originalSlip.getCompany()) + "-BO");
  ...
  public Long resolveBackorderSlipIdForResponse(
      PackagingSlip slip, Company company, boolean hasBackorder) {
  ...
  private void syncOrderStatusAfterBackorderCancellation(Company company, SalesOrder salesOrder) {
    ...
    SalesOrder managedOrder =
        salesOrderRepository
            .findWithItemsByCompanyAndIdForUpdate(company, salesOrder.getId())
            .orElse(null);
    ...
    String nextStatus =
        normalizeBackorderCancellationTargetStatus(
            resolveOrderStatusFromSlips(company, managedOrder));
  ...
  private boolean isBackorderCancellationStatusSyncAllowed(SalesOrder order) {
    ...
    boolean allowedStatus =
        "PENDING_PRODUCTION".equalsIgnoreCase(status)
  ...
  private String resolveOrderStatusFromSlips(Company company, SalesOrder order) {
    ...
    List<PackagingSlip> activeSlips =
        slips.stream().filter(slip -> !"CANCELLED".equalsIgnoreCase(slip.getStatus())).toList();
    ...
    boolean anyDispatched =
        activeSlips.stream().anyMatch(slip -> "DISPATCHED".equalsIgnoreCase(slip.getStatus()));
    if (!anyDispatched) {
      return order.getStatus();
    }
  ...
  private boolean canTransitionStatus(String current, String target) {
    ...
    return switch (from) {
      case "PENDING" ->
          List.of("PENDING", "PENDING_STOCK", "PENDING_PRODUCTION", "RESERVED").contains(to);
      case "PENDING_STOCK" ->
          List.of("PENDING_STOCK", "PENDING_PRODUCTION", "RESERVED").contains(to);
