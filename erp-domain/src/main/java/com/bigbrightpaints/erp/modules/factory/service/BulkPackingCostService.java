...
public class BulkPackingCostService {
  ...
  public BulkPackCostSummary consumePackagingIfRequired(
      Company company, BulkPackRequest request, String packReference) {
    if (request == null) {
  ...
  public BulkPackCostingContext createCostingContext(
      BigDecimal bulkUnitCost, BulkPackCostSummary packagingCostSummary, int totalPacks) {
    BigDecimal fallbackPackagingCostPerUnit =
        totalPacks > 0
            ? packagingCostSummary
                .totalCost()
                .divide(BigDecimal.valueOf(totalPacks), 6, COST_ROUNDING)
            : BigDecimal.ZERO;
    ...
    return new BulkPackCostingContext(
        bulkUnitCost, fallbackPackagingCostPerUnit, lineCosts, hasLineCosts);
  }

  public BigDecimal resolveLinePackagingCostPerUnit(
      BulkPackCostingContext context, BulkPackRequest.PackLine line, int lineIndex) {
  ...
  private BulkPackCostSummary consumePackagingFromMappings(
      Company company, List<BulkPackRequest.PackLine> packs, String reference) {
    ...
    for (BulkPackRequest.PackLine line : packs) {
      ...
      PackagingConsumptionResult result =
          packagingMaterialService.consumePackagingMaterial(sizeLabel, piecesCount, reference);
      ...
      result
          .accountTotalsOrEmpty()
          .forEach((accountId, amount) -> accountTotals.merge(accountId, amount, BigDecimal::add));
    }
