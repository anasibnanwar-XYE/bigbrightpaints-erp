...
public class FinishedGoodsReservationEngine {
  ...
  public FinishedGoodsReservationEngine(
  ...
  public FinishedGoodsService.InventoryReservationResult reserveForOrder(SalesOrder order) {
    ...
    SalesOrder managedOrder =
        salesOrderRepository
            .findWithItemsByCompanyAndIdForUpdate(company, order.getId())
            .orElseThrow(
                () ->
                    com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
                        "Sales order not found: " + order.getId()));

    List<PackagingSlip> lockedSlips =
        packagingSlipRepository.findAllByCompanyAndSalesOrderIdForUpdate(company, order.getId());
    List<PackagingSlip> primarySlips =
        lockedSlips.stream().filter(existing -> !existing.isBackorder()).toList();
    if (primarySlips.size() > 1) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "Ambiguous primary packaging slip state for order " + order.getId());
    }
    ...
    return new FinishedGoodsService.InventoryReservationResult(
        toSlipDto(slip), List.copyOf(shortages));
  }
  ...
  private boolean synchronizeReservationsForSlip(PackagingSlip slip, SalesOrder order) {
    ...
    for (PackagingSlipLine line : slip.getLines()) {
      ...
      expectedByBatchId.merge(
          batch.getId(),
          inventoryValuationService.safeQuantity(line.getQuantity()),
          BigDecimal::add);
    }

    List<InventoryReservation> reservations =
        inventoryReservationRepository.findByFinishedGoodCompanyAndReferenceTypeAndReferenceId(
            slip.getCompany(), InventoryReference.SALES_ORDER, order.getId().toString());

    List<InventoryReservation> activeReservations =
        reservations.stream().filter(this::isActiveReservation).toList();

    if (activeReservations.isEmpty()) {
      if (isTerminalDispatchReplay(slip, reservations, expectedByBatchId)) {
        recalculateReservationState(
            lockTouchedGoods(slip.getCompany(), reservations), lockTouchedBatches(reservations));
        return true;
      }
    ...
    for (InventoryReservation reservation : reservations) {
      ...
      reservationsByBatchId
          .computeIfAbsent(batch.getId(), ignored -> new ArrayList<>())
          .add(reservation);
    }
    ...
    for (Map.Entry<Long, List<InventoryReservation>> entry : reservationsByBatchId.entrySet()) {
      ...
      List<InventoryReservation> activeForBatch =
          batchReservations.stream().filter(this::isActiveReservation).toList();
      ...
      for (InventoryReservation reservation : batchReservations) {
        ...
        if (finishedGood != null && finishedGood.getId() != null) {
          touchedGoods.computeIfAbsent(
              finishedGood.getId(), id -> lockFinishedGood(slip.getCompany(), id));
        }
      }
      touchedBatches.computeIfAbsent(
          batchId,
          id ->
                  ...
                  .orElseThrow(
                      () ->
                          com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
                              "Finished good batch not found: " + id)));
    }
  ...
  private void applySynchronizedAllocation(
    ...
    if (fulfilled.compareTo(BigDecimal.ZERO) > 0
        || "PARTIAL".equalsIgnoreCase(reservation.getStatus())) {
  ...
  private boolean isTerminalDispatchReplay(
    ...
    Map<Long, BigDecimal> fulfilledByBatchId =
        fulfilledReservationQuantitiesByBatchId(reservations);
  ...
  private Map<Long, BigDecimal> fulfilledReservationQuantitiesByBatchId(
    ...
    for (InventoryReservation reservation : reservations) {
      ...
      fulfilledByBatchId.merge(
          batch.getId(),
          inventoryValuationService.safeQuantity(reservation.getQuantity()),
          BigDecimal::add);
    }
  ...
  private Map<Long, BigDecimal> dispatchedMovementQuantitiesByBatchId(PackagingSlip slip) {
    ...
    for (InventoryMovement movement : dispatchMovements) {
      if (movement == null
          ...
          || movement.getFinishedGoodBatch().getId() == null) {
        return Map.of();
      }
  ...
  public void releaseReservationsForOrder(Long orderId) {
    ...
    List<Long> ids =
        reservations.stream().map(r -> r.getFinishedGood().getId()).distinct().sorted().toList();
  ...
  @Transactional
  public List<InventoryReservation> rebuildReservationsFromSlip(
      PackagingSlip slip, Long salesOrderId) {
    if (slip.getLines() == null || slip.getLines().isEmpty()) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          "No packaging slip lines available to rebuild reservations for order " + salesOrderId);
    }
    ...
    for (PackagingSlipLine line : slip.getLines()) {
      ...
      if (batch == null || batch.getFinishedGood() == null) {
        throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
            "Cannot rebuild reservation without a finished good batch");
      }
      ...
      if (batch.getId() != null) {
        touchedBatches.computeIfAbsent(
            batch.getId(), id -> finishedGoodBatchRepository.lockById(id).orElse(batch));
      }
  ...
  private void recalculateReservationState(
      Map<Long, FinishedGood> touchedGoods, Map<Long, FinishedGoodBatch> touchedBatches) {
    if (!touchedGoods.isEmpty()) {
      for (FinishedGood fg : touchedGoods.values()) {
        BigDecimal reservedTotal =
            inventoryReservationRepository.findByFinishedGood(fg).stream()
    ...
    if (!touchedBatches.isEmpty()) {
      for (FinishedGoodBatch batch : touchedBatches.values()) {
        BigDecimal reservedForBatch =
            inventoryReservationRepository.findByFinishedGoodBatch(batch).stream()
                .map(this::resolveReservedQuantity)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
  ...
  public boolean slipLinesMatchOrder(PackagingSlip slip, SalesOrder order) {
    ...
    for (PackagingSlipLine line : slip.getLines()) {
      if (line.getFinishedGoodBatch() == null
  ...
  private void allocateItem(
    ...
    for (FinishedGoodBatch batch : batches) {
      ...
      finishedGood.setReservedStock(
          inventoryValuationService.safeQuantity(finishedGood.getReservedStock()).add(allocation));
    ...
    if (remaining.compareTo(BigDecimal.ZERO) > 0) {
      shortages.add(
          new FinishedGoodsService.InventoryShortage(
              finishedGood.getProductCode(), remaining, finishedGood.getName()));
    }
  }
    ...
    CostingMethodUtils.FinishedGoodBatchSelectionMethod selectionMethod =
        CostingMethodUtils.resolveFinishedGoodBatchSelectionMethod(
            costingMethodService
                .resolveActiveMethod(finishedGood.getCompany(), referenceDate)
                .name());
  ...
  private PackagingSlipDto toSlipDto(PackagingSlip slip) {
    List<com.bigbrightpaints.erp.modules.inventory.dto.PackagingSlipLineDto> lines =
