...
public class PayrollRunService {
  ...
  public PayrollRunDto createPayrollRun(CreatePayrollRunRequest request) {
    ...
    if (request == null) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_MISSING_REQUIRED_FIELD, "Payroll run request is required");
    }
    if (request.runType() == null) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_MISSING_REQUIRED_FIELD, "Payroll run type is required");
    }
    ...
    String idempotencyKey =
        buildIdempotencyKey(request.runType(), request.periodStart(), request.periodEnd());
    String requestSignature =
        buildRunSignature(
            request.runType(), request.periodStart(), request.periodEnd(), request.remarks());

    Optional<PayrollRun> existing =
        payrollRunRepository.findByCompanyAndIdempotencyKey(company, idempotencyKey);
    ...
    try {
      ...
      Optional<PayrollRun> concurrent =
          payrollRunRepository.findByCompanyAndIdempotencyKey(company, idempotencyKey);
  ...
  public PayrollRunDto createWeeklyPayrollRun(LocalDate weekEndingDate) {
    ...
    return createPayrollRun(
        new CreatePayrollRunRequest(
            PayrollRun.RunType.WEEKLY, weekStart, weekEnd, "Weekly payroll for labourers"));
  }
  ...
  public static String buildIdempotencyKey(
    ...
    return "PAYROLL:%s:%s:%s".formatted(runType.name(), periodStart, periodEnd);
  }

  public static String buildRunSignature(
  ...
  public static String buildRunSignature(PayrollRun run) {
    if (run == null
        || run.getRunType() == null
        || run.getPeriodStart() == null
        || run.getPeriodEnd() == null) {
      return null;
    }
    return buildRunSignature(
        run.getRunType(), run.getPeriodStart(), run.getPeriodEnd(), run.getNotes());
  }

  private void assertRunSignatureMatches(
      PayrollRun run, String expectedSignature, String idempotencyKey) {
    if (run == null) {
      return;
    }
