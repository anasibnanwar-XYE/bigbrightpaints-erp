...
public class ClosedPeriodPostingExceptionService {
  ...
  public ClosedPeriodPostingException authorize(
      Company company,
    ...
    if (authentication == null || !authentication.isAuthenticated()) {
      throw new ApplicationException(
          ErrorCode.AUTH_INSUFFICIENT_PERMISSIONS, ADMIN_EXCEPTION_REQUIRED);
    }
    boolean admin =
        authentication.getAuthorities().stream()
            .anyMatch(
                authority -> {
                  ...
                  return "ROLE_ADMIN".equalsIgnoreCase(name)
                      || "ROLE_SUPER_ADMIN".equalsIgnoreCase(name);
                });
    if (!admin) {
      throw new ApplicationException(
          ErrorCode.AUTH_INSUFFICIENT_PERMISSIONS, ADMIN_EXCEPTION_REQUIRED);
    ...
    ClosedPeriodPostingException exception =
        repository
            .findByCompanyAndDocumentTypeIgnoreCaseAndDocumentReferenceIgnoreCaseOrderByApprovedAtDescIdDesc(
                company, normalizedDocumentType, normalizedDocumentReference)
            .stream()
            .filter(
                existing -> existing.getExpiresAt() != null && existing.getExpiresAt().isAfter(now))
            .findFirst()
  ...
  public void linkJournalEntry(
      Company company, String documentType, String documentReference, JournalEntry journalEntry) {
    if (company == null
        || journalEntry == null
        || !StringUtils.hasText(documentType)
        || !StringUtils.hasText(documentReference)) {
      return;
    }
    repository
        .findByCompanyAndDocumentTypeIgnoreCaseAndDocumentReferenceIgnoreCaseOrderByApprovedAtDescIdDesc(
            company, documentType.trim(), documentReference.trim())
        .stream()
        .findFirst()
        .ifPresent(
            exception -> {
