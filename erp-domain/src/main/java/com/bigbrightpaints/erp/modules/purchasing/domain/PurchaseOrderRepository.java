...
public interface PurchaseOrderRepository extends JpaRepository<PurchaseOrder, Long> {
  ...
  @EntityGraph(attributePaths = {"supplier", "lines", "lines.rawMaterial"})
  @Query(
      "select po from PurchaseOrder po where po.company = :company and po.supplier = :supplier"
          + " order by po.orderDate desc")
  List<PurchaseOrder> findByCompanyAndSupplierWithLinesOrderByOrderDateDesc(
  ...
  Optional<PurchaseOrder> findByCompanyAndOrderNumberIgnoreCase(
      Company company, String orderNumber);
  ...
  @Query(
      ...
      where po.company = :company
        and po.id = :id
      """)
  Optional<PurchaseOrder> lockByCompanyAndId(
      @Param("company") Company company, @Param("id") Long id);

  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Query(
      "select po from PurchaseOrder po where po.company = :company and lower(po.orderNumber) ="
          + " lower(:orderNumber)")
  Optional<PurchaseOrder> lockByCompanyAndOrderNumberIgnoreCase(
      @Param("company") Company company, @Param("orderNumber") String orderNumber);
}
