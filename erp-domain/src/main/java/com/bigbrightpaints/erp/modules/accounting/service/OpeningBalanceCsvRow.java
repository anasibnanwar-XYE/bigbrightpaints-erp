...
record OpeningBalanceCsvRow(
  ...
  static OpeningBalanceCsvRow from(CSVRecord record) {
    ...
    if (debitAmount.compareTo(BigDecimal.ZERO) == 0
      ...
      throw ValidationUtils.invalidInput(
          "Either debit_amount or credit_amount must be greater than zero for account_code "
              + normalizedCode);
    }
    if (debitAmount.compareTo(BigDecimal.ZERO) > 0 && creditAmount.compareTo(BigDecimal.ZERO) > 0) {
      throw ValidationUtils.invalidInput(
          "Both debit_amount and credit_amount cannot be non-zero for account_code "
              + normalizedCode);
    }
  ...
  private static AccountType parseAccountType(String rawValue) {
    String normalized =
        ValidationUtils.requireNotBlank(rawValue, "account_type").toUpperCase(Locale.ROOT);
  ...
  private static BigDecimal parseAmount(String rawValue, String fieldName) {
    ...
    try {
      ...
      throw ValidationUtils.invalidInput(
          "Invalid numeric value for " + fieldName + ": " + rawValue);
    }
  }
