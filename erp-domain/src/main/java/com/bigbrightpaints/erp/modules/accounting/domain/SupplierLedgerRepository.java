...
public interface SupplierLedgerRepository extends JpaRepository<SupplierLedgerEntry, Long> {
  ...
  List<SupplierLedgerEntry> findByCompanyAndSupplierOrderByEntryDateAsc(
  ...
  List<SupplierBalanceView> aggregateBalancesBetween(
  ...
  @Query(
      "select new com.bigbrightpaints.erp.modules.accounting.dto.SupplierBalanceView(e.supplier.id,"
          ...
          + " e.supplier.id")
  List<SupplierBalanceView> aggregateBalancesUpTo(
      @Param("company") Company company,
      @Param("supplierIds") Collection<Long> supplierIds,
      @Param("end") java.time.LocalDate end);
  ...
  List<SupplierLedgerEntry> findByCompanyAndSupplierAndEntryDateBetweenOrderByEntryDateAscIdAsc(
      Company company, Supplier supplier, java.time.LocalDate start, java.time.LocalDate end);

  List<SupplierLedgerEntry> findByCompanyAndSupplierAndEntryDateBetweenOrderByEntryDateAsc(
      Company company, Supplier supplier, java.time.LocalDate start, java.time.LocalDate end);

  List<SupplierLedgerEntry> findByCompanyAndSupplierAndEntryDateBeforeOrderByEntryDateAsc(
      Company company, Supplier supplier, java.time.LocalDate before);

  List<SupplierLedgerEntry>
      findByCompanyAndSupplierAndEntryDateLessThanEqualOrderByEntryDateAscIdAsc(
          Company company, Supplier supplier, java.time.LocalDate asOf);
}
