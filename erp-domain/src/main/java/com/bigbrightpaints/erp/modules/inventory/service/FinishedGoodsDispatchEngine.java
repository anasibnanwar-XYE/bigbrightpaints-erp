...
public class FinishedGoodsDispatchEngine {
  ...
  public FinishedGoodsDispatchEngine(
  ...
  public List<FinishedGoodsService.DispatchPosting> markSlipDispatched(Long salesOrderId) {
    ...
    List<PackagingSlip> slips =
        packagingSlipRepository.findPrimarySlipsByOrderId(company, salesOrderId);
    if (slips.isEmpty()) {
    ...
    PackagingSlip slip =
        packagingSlipRepository
            .findAndLockByIdAndCompany(selected.getId(), company)
            .orElseThrow(
                () ->
  ...
  public List<FinishedGoodsService.DispatchPosting> markSlipDispatched(
    ...
    for (PackagingSlipLine slipLine : slip.getLines()) {
      ...
      BigDecimal ordered =
          slipLine.getOrderedQuantity() != null
              ? slipLine.getOrderedQuantity()
              : slipLine.getQuantity();
      if (ordered == null) {
        ordered = BigDecimal.ZERO;
      }
      BigDecimal shippedSoFar =
          inventoryValuationService.safeQuantity(slipLine.getShippedQuantity());
    ...
    List<InventoryReservation> reservations =
        inventoryReservationRepository.findByFinishedGoodCompanyAndReferenceTypeAndReferenceId(
            slip.getCompany(), InventoryReference.SALES_ORDER, salesOrderId.toString());
    if (reservations.isEmpty()) {
      reservations = reservationEngine.rebuildReservationsFromSlip(slip, salesOrderId);
      if (reservations.isEmpty()) {
        throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
            "No reservations found for order " + salesOrderId);
      }
    }

    Map<Long, FinishedGood> lockedGoods =
        lockFinishedGoodsInOrder(
            slip.getCompany(),
            reservations.stream()
    ...
    for (InventoryReservation reservation : reservations) {
      ...
      BigDecimal unitCost = inventoryValuationService.resolveDispatchUnitCost(fg, batch);
      ...
      requireSufficientQuantity(
          reserved, shipQty, "Reserved stock insufficient for FG " + fg.getProductCode());
      ...
      if (batch != null) {
        ...
        requireSufficientQuantity(
            batchTotal,
            shipQty,
            "Batch stock insufficient for batch "
                + batch.getBatchCode()
                + " FG "
                + fg.getProductCode());
      ...
      if (batch != null) {
        ...
        if (batchLines != null && shipQty.compareTo(BigDecimal.ZERO) > 0) {
          ...
          for (PackagingSlipLine line : batchLines) {
            ...
            BigDecimal shippedSoFar =
                inventoryValuationService.safeQuantity(line.getShippedQuantity());
    ...
    boolean anyShipped =
        reservations.stream()
            .anyMatch(
                r ->
                    r.getFulfilledQuantity() != null
                        && r.getFulfilledQuantity().compareTo(BigDecimal.ZERO) > 0);
    boolean anyPending =
        reservations.stream()
            .anyMatch(
                r ->
                    !"FULFILLED".equalsIgnoreCase(r.getStatus())
    ...
    return postingBuilders.values().stream().map(DispatchPostingBuilder::build).toList();
  }
  ...
  public DispatchPreviewDto getDispatchPreview(Long packagingSlipId) {
    ...
    Map<Long, BigDecimal> reservedByBatch =
    ...
    String companyStateCode =
        inventoryValuationService.normalizeStateCode(
            company != null ? company.getStateCode() : null);
    String dealerStateCode =
        inventoryValuationService.normalizeStateCode(dealer != null ? dealer.getStateCode() : null);
    ...
    for (PackagingSlipLine line : slip.getLines()) {
      ...
      BigDecimal ordered =
          inventoryValuationService.safeQuantity(
              line.getOrderedQuantity() != null ? line.getOrderedQuantity() : line.getQuantity());
      ...
      SalesOrderItem matchedItem =
          pollMatchingOrderItem(orderItemsByProductCode, fg.getProductCode());
      BigDecimal unitPrice =
          matchedItem != null && matchedItem.getUnitPrice() != null
              ? matchedItem.getUnitPrice()
              : BigDecimal.ZERO;
      BigDecimal lineSubtotal =
          inventoryValuationService.currency(unitPrice.multiply(suggestedShip));
      BigDecimal lineRate =
          matchedItem != null
      ...
      GstService.GstBreakdown lineBreakdown =
          lineRate.compareTo(BigDecimal.ZERO) > 0
              ? gstService.splitTaxAmount(
                  ...
                  dealerStateCode)
              : gstService.splitTaxAmount(
                  lineSubtotal, BigDecimal.ZERO, companyStateCode, dealerStateCode);

      BigDecimal lineTax =
          inventoryValuationService
              .safeQuantity(lineBreakdown.cgst())
      ...
      totalOrdered =
          totalOrdered.add(inventoryValuationService.currency(unitPrice.multiply(ordered)));
    ...
    DispatchPreviewDto.GstBreakdown breakdown =
  ...
  public DispatchConfirmationResponse confirmDispatch(
    ...
    PackagingSlip slip =
        packagingSlipRepository
            .findAndLockByIdAndCompany(request.packagingSlipId(), company)
            .orElseThrow(
                () ->
                    com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
                        "Packaging slip not found"));
    ...
    SalesOrder order = slip.getSalesOrder();
    Map<Long, DispatchConfirmationRequest.LineConfirmation> confirmations =
        request.lines().stream()
            .collect(
                Collectors.toMap(DispatchConfirmationRequest.LineConfirmation::lineId, l -> l));
    ...
    if (order != null) {
      ...
      for (List<InventoryReservation> batchReservations : reservationsByBatch.values()) {
        batchReservations.sort(
            Comparator.comparing(
                InventoryReservation::getId, Comparator.nullsLast(Long::compareTo)));
      }
    }
    ...
    for (PackagingSlipLine line : slip.getLines()) {
      ...
      if (conf == null) {
        throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
            "Missing confirmation for line " + line.getId());
      }
        ...
        lockedGoods.put(
            ...
            finishedGoodRepository
                ...
                .orElseThrow(
                    () ->
                        com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
                            "Finished good not found: " + originalFg.getId())));
      }
      ...
      if (shipped.compareTo(BigDecimal.ZERO) > 0) {
        ...
        if (currentStock.compareTo(shipped) < 0) {
          throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
              "Insufficient current stock for FG "
                  + fg.getProductCode()
                  + ": available="
                  + currentStock
                  + ", requested="
        ...
        if (batch.getId() != null) {
          ...
          requireSufficientQuantity(
              batchReserved,
              shipped,
              "Reserved quantity insufficient for batch "
                  + batch.getBatchCode()
                  + " FG "
                  + fg.getProductCode());
        ...
        requireSufficientQuantity(
            reservedStock, shipped, "Reserved stock insufficient for FG " + fg.getProductCode());
  ...
  private void applyReservationFulfillment(
      Map<Long, List<InventoryReservation>> reservationsByBatch,
      FinishedGoodBatch batch,
      BigDecimal shipped) {
    if (batch == null
        || batch.getId() == null
        || shipped == null
        || shipped.compareTo(BigDecimal.ZERO) <= 0) {
      return;
    }
    ...
    for (InventoryReservation reservation : reservations) {
      ...
      BigDecimal fulfilled =
          inventoryValuationService.safeQuantity(reservation.getFulfilledQuantity());
  ...
  private void updateReservationStatuses(List<InventoryReservation> reservations) {
    for (InventoryReservation reservation : reservations) {
      ...
      BigDecimal reservedQty =
          inventoryValuationService.safeQuantity(reservation.getReservedQuantity());
  ...
  private DispatchConfirmationResponse buildDispatchConfirmationResponse(
      PackagingSlip slip, Company company) {
    List<DispatchConfirmationResponse.LineResult> lineResults = new ArrayList<>();
    BigDecimal totalOrdered = BigDecimal.ZERO;
    BigDecimal totalShipped = BigDecimal.ZERO;
    BigDecimal totalBackorder = BigDecimal.ZERO;
    ...
    Long backorderSlipId =
        packagingSlipService.resolveBackorderSlipIdForResponse(slip, company, hasBackorder);
      ...
      FinishedGood fg =
          finishedGoodRepository
              ...
              .orElseThrow(
                  () ->
                      com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
                          "Finished good not found: " + id));
      locked.put(id, fg);
    }
    return locked;
  }

  private SalesOrderItem pollMatchingOrderItem(
      Map<String, Deque<SalesOrderItem>> orderItemsByProductCode, String productCode) {
  ...
  private void requireSufficientQuantity(
      BigDecimal available, BigDecimal required, String context) {
    ...
    if (safeAvailable.compareTo(safeRequired) < 0) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidState(
          context + ": available=" + safeAvailable + ", requested=" + safeRequired);
    }
  }
