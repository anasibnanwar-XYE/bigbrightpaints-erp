...
public class PurchaseReturnAllocationService {
  ...
  public BigDecimal remainingReturnableQuantity(
      RawMaterialPurchase purchase, RawMaterial material) {
    ...
    if (remaining.compareTo(BigDecimal.ZERO) < 0
        && remaining.abs().compareTo(QUANTITY_TOLERANCE) <= 0) {
      return BigDecimal.ZERO;
    }
    return remaining.max(BigDecimal.ZERO);
  }

  public void applyPurchaseReturnQuantity(
      RawMaterialPurchase purchase, RawMaterial material, BigDecimal quantity) {
    if (purchase == null
        || material == null
        || quantity == null
  ...
  public void applyPurchaseReturnToOutstanding(
      RawMaterialPurchase purchase, BigDecimal totalAmount) {
    if (purchase == null) {
      return;
    }
    ...
    BigDecimal currentOutstanding =
        currency(MoneyUtils.zeroIfNull(purchase.getOutstandingAmount()));
    BigDecimal newOutstanding = currency(currentOutstanding.subtract(amount));
    if (newOutstanding.compareTo(BigDecimal.ZERO) < 0) {
      throw new ApplicationException(
              ErrorCode.RETURN_EXCEEDS_OUTSTANDING,
    ...
    if (purchase.getOutstandingAmount().compareTo(BigDecimal.ZERO) == 0
        && isPurchaseFullyReturned(purchase)) {
