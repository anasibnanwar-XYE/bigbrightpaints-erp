...
public class AccountingConfigurationController {
    ...
    @GetMapping("/health")
    public ResponseEntity<ApiResponse<ConfigurationHealthService.ConfigurationHealthReport>> health() {
        return ResponseEntity.ok(ApiResponse.success("Configuration health report", configurationHealthService.evaluate()));
    }
}
