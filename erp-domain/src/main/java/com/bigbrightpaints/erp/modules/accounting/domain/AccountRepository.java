...
public interface AccountRepository extends JpaRepository<Account, Long> {
  ...
  @Query(
      "UPDATE Account a SET a.balance = a.balance + :delta WHERE a.company = :company AND a.id ="
          + " :id")
  ...
  List<Account> findByCompanyAndParentOrderByCodeAsc(
      Company company, Account parent); // Children of parent

  @Query(
      "SELECT a FROM Account a WHERE a.company = :company AND a.parent.id = :parentId ORDER BY"
          + " a.code")
