...
public class AgedDebtorsReportQueryService {
  ...
  public AgedDebtorsReportQueryService(
      ReportQuerySupport reportQuerySupport, DealerLedgerRepository dealerLedgerRepository) {
  ...
  public List<AgedDebtorDto> generate(FinancialReportQueryRequest request) {
    ...
    List<DealerLedgerEntry> unpaidEntries =
        dealerLedgerRepository.findAllUnpaidAsOf(window.company(), asOfDate);
    Map<Long, Bucket> bucketsByDealer = new LinkedHashMap<>();

    for (DealerLedgerEntry entry : unpaidEntries) {
      if (filterByDateRange
          && !withinWindow(entry.getEntryDate(), window.startDate(), window.endDate())) {
        continue;
      }
      ...
      Bucket bucket =
          bucketsByDealer.computeIfAbsent(dealer.getId(), ignored -> new Bucket(dealer));
  ...
  private static class Bucket {
    ...
    private AgedDebtorDto toDto(ReportMetadata metadata, ExportHints exportHints) {
      BigDecimal total =
          current.add(oneToThirty).add(thirtyOneToSixty).add(sixtyOneToNinety).add(ninetyPlus);
