...
public class PurchaseTaxPolicy {
  ...
  public PurchaseTaxMode resolvePurchaseTaxMode(
      List<RawMaterialPurchaseLineRequest> lineRequests, Map<Long, RawMaterial> lockedMaterials) {
    ...
    for (RawMaterialPurchaseLineRequest lineRequest : lineRequests) {
      ...
      PurchaseTaxMode lineMode =
          rawMaterial.isGstApplicable() ? PurchaseTaxMode.GST : PurchaseTaxMode.NON_GST;
      if (resolved == null) {
        resolved = lineMode;
        continue;
      }
  ...
  public BigDecimal resolveLineTaxRateForMode(
      RawMaterialPurchaseLineRequest lineRequest,
  ...
  public void enforcePurchaseTaxContract(
      PurchaseTaxMode purchaseTaxMode, BigDecimal providedTaxAmount, boolean hasTaxableLines) {
    ...
    if (purchaseTaxMode == PurchaseTaxMode.GST
      ...
      throw new ApplicationException(
              ErrorCode.VALIDATION_INVALID_INPUT,
              "GST purchase invoice with taxable lines requires non-zero taxAmount or tax"
                  + " auto-computation")
          .withDetail("taxMode", taxModeLabel(purchaseTaxMode))
          .withDetail("taxAmount", providedTaxAmount);
    }
  ...
  private void enforceNonGstLineContract(
      RawMaterialPurchaseLineRequest lineRequest, RawMaterial rawMaterial) {
    BigDecimal requestedTaxRate = lineRequest != null ? lineRequest.taxRate() : null;
    if (requestedTaxRate != null
        && normalizePercent(requestedTaxRate).compareTo(BigDecimal.ZERO) > 0) {
      throw new ApplicationException(
              ErrorCode.VALIDATION_INVALID_INPUT,
    ...
    if (lineRequest != null && Boolean.TRUE.equals(lineRequest.taxInclusive())) {
      throw new ApplicationException(
              ErrorCode.VALIDATION_INVALID_INPUT, "Non-GST purchase line cannot be tax-inclusive")
          .withDetail("rawMaterialId", rawMaterial != null ? rawMaterial.getId() : null);
    }
    BigDecimal materialTaxRate =
        rawMaterial != null ? normalizePercent(rawMaterial.getGstRate()) : BigDecimal.ZERO;
  ...
  private BigDecimal resolveLineTaxRate(
      RawMaterialPurchaseLineRequest lineRequest, RawMaterial rawMaterial, Company company) {
  ...
  private BigDecimal normalizePercent(BigDecimal rate) {
    ...
    if (rate.compareTo(BigDecimal.ZERO) < 0) {
      throw com.bigbrightpaints.erp.core.validation.ValidationUtils.invalidInput(
          "GST rate must be zero or positive");
    }
