...
public class SalaryStructureTemplateService {
  ...
  public SalaryStructureTemplateService(
  ...
  public List<SalaryStructureTemplateDto> listTemplates() {
    ...
    return salaryStructureTemplateRepository.findByCompanyOrderByNameAsc(company).stream()
        .map(this::toDto)
        .toList();
  }
  ...
  public SalaryStructureTemplateDto createTemplate(SalaryStructureTemplateRequest request) {
    ...
    salaryStructureTemplateRepository
        .findByCompanyAndCodeIgnoreCase(company, request.code().trim())
        .ifPresent(
            existing -> {
              throw new ApplicationException(
  ...
  @Transactional
  public SalaryStructureTemplateDto updateTemplate(
    ...
    if (!incomingCode.equalsIgnoreCase(template.getCode())) {
      salaryStructureTemplateRepository
          .findByCompanyAndCodeIgnoreCase(company, incomingCode)
          .ifPresent(
              existing -> {
                throw new ApplicationException(
                        ErrorCode.BUSINESS_DUPLICATE_ENTRY,
  ...
  private void applyMutableFields(
      SalaryStructureTemplate template, SalaryStructureTemplateRequest request) {
    ...
    template.setEsiEligibilityThreshold(
        safeMoney(request.esiEligibilityThreshold(), new BigDecimal("21000.00")));
    ...
    if (total.compareTo(BigDecimal.ZERO) <= 0) {
      throw new ApplicationException(
          ErrorCode.VALIDATION_INVALID_INPUT, "Salary structure must have positive total earnings");
    }
  }
  ...
  private BigDecimal safeMoney(BigDecimal value, BigDecimal defaultValue) {
    ...
    if (value.compareTo(BigDecimal.ZERO) < 0) {
      throw new ApplicationException(
              ErrorCode.VALIDATION_OUT_OF_RANGE, "Salary components cannot be negative")
  ...
  private BigDecimal safeRate(BigDecimal value, BigDecimal defaultValue) {
    ...
    if (value.compareTo(BigDecimal.ZERO) < 0) {
      throw new ApplicationException(ErrorCode.VALIDATION_OUT_OF_RANGE, "Rate cannot be negative")
          .withDetail("value", value);
    }
    return value;
  }
