...
- Historical note: the old flat Super Admin tenant onboarding route (`POST /api/v1/superadmin/tenants/onboard`) is retired and must not be used as a current create-tenant contract. Current frontend work must use the canonical Add Client + activation contract in `docs/frontend-portals/superadmin/**` and `docs/frontend-api/**`.
...
- Historical note: the retired flat onboarding route no longer belongs in current-contract prose; it returns the documented retirement response and does not expose plaintext password fields or `credentialsEmailSent`.
...
| `control-plane-approval-bootstrap-contract-cleanup` | Admin and tenant control plane | Required contract review | Switch tenant-scoped approval consumers to `originType`/`ownerType`, remove any onboarding plaintext-password UI, and use the canonical Add Client + activation contract instead of the retired flat onboarding route | [control-plane-approval-bootstrap-contract-cleanup.md](./control-plane-approval-bootstrap-contract-cleanup.md) |
