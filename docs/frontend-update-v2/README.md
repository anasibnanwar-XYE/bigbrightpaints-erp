...
- Tenant-admin foreign-target status, lock/unlock, session-revoke, and `mfa/disable` flows keep the same masked `400 User not found` contract; retired suspend, unsuspend, and hard-delete aliases are absent/non-mutating.
