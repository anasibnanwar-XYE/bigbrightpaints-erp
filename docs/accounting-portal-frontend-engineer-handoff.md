...
- Exact error conditions frontend must handle here: missing explicit idempotency key, idempotency replay conflict, missing `OPEN-BAL`, `OPEN-BAL` not equity, missing SKU, `stage=catalog` failures (`PRODUCT_MASTER_MISSING`, `PRODUCT_INACTIVE`), and `stage=inventory` failures (`RAW_MATERIAL_CATEGORY_REQUIRED`, `RAW_MATERIAL_MIRROR_MISSING`, `RAW_MATERIAL_INVENTORY_ACCOUNT_MISSING`, `FINISHED_GOOD_CATEGORY_REQUIRED`, `FINISHED_GOOD_MIRROR_MISSING`, `FINISHED_GOOD_VALUATION_ACCOUNT_MISSING`, `FINISHED_GOOD_COGS_ACCOUNT_MISSING`, `FINISHED_GOOD_REVENUE_ACCOUNT_MISSING`, `FINISHED_GOOD_TAX_ACCOUNT_MISSING`).
