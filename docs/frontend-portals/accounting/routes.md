...
| `/accounting/period-close` | `GET /api/v1/accounting/periods`, `POST /api/v1/accounting/periods/{periodId}/request-close` | Accounting creates close requests and shows close status. Approval/rejection actions belong to tenant-admin via `POST /api/v1/admin/approvals/{originType}/{id}/decisions`. |
