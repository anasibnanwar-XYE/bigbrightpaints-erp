...
- `lockUser(...)` and `unlockUser(...)` are the canonical lock-state controls; retired suspend, unsuspend, and hard-delete aliases are absent/non-mutating.
...
| medium | internal-boundary drift | `AdminUserService` still contains broad scoped-target helpers even though the canonical `/api/v1/admin/users/**` surface is tenant-admin only and retired suspend/unsuspend/hard-delete aliases are absent. | `AdminUserService`, `AdminUserController`, `CompanyContextFilter` | A future caller could accidentally re-expose cross-tenant admin-user behavior if it bypasses the current controller/filter boundary. |
| low | resolved hard-cut cleanup | Retired suspend/unsuspend/hard-delete user aliases are absent/non-mutating; canonical status, lock/unlock, session revocation, and MFA-disable routes remain. | `AdminUserController`, `IdentityRouteInventoryContractTest`, `openapi.json` | Client and worker guidance should continue using only the canonical route set to avoid restoring duplicate lifecycle semantics. |
