...
| --- | --- | --- | --- |
| Admin user governance | `GET/POST /api/v1/admin/users`, `PUT /api/v1/admin/users/{id}`, `PUT /api/v1/admin/users/{userId}/status`, `POST /api/v1/admin/users/{userId}/lock`, `POST /api/v1/admin/users/{userId}/unlock`, `POST /api/v1/admin/users/{userId}/force-reset-password`, `PATCH /api/v1/admin/users/{id}/mfa/disable`, `DELETE /api/v1/admin/users/{userId}/sessions`, `GET /api/v1/admin/users/{userId}/security-events`, `GET /api/v1/admin/users/assignable-roles` | `AdminUserController` | Tenant-admin operational surface only. Platform super admins are blocked from this workflow prefix and must stay on `/api/v1/superadmin/tenants/**`; retired `suspend`, `unsuspend`, and `DELETE /api/v1/admin/users/{userId}` aliases are absent/non-mutating. |
...
- `updateUserStatus(...)` uses quota checks when re-enabling a user, revokes all tokens on status disablement, sends status-change email, and audits the action.
- `disableMfa(...)` clears the MFA secret and recovery codes, then revokes all tokens.
- `lockUser(...)` and `unlockUser(...)` are the canonical lock-state controls; retired `suspend`, `unsuspend`, and `DELETE /api/v1/admin/users/{userId}` aliases are absent/non-mutating.
...
- Tenant admins get masked-as-missing responses for out-of-scope users across update, reset, status, lock/unlock, session revocation, and MFA-disable flows.
...
- User status disable/reactivate, lock-triggered/session revocation, force password reset, and MFA disable/reset flows revoke access and refresh tokens as required and may send outbound email.
...
| medium | internal-boundary drift | `AdminUserService` still contains broad scoped-target helpers even though the canonical `/api/v1/admin/users/**` surface is tenant-admin only and retired `suspend`, `unsuspend`, and `DELETE /api/v1/admin/users/{userId}` aliases are absent. | `AdminUserService`, `AdminUserController`, `CompanyContextFilter` | A future caller could accidentally re-expose cross-tenant admin-user behavior if it bypasses the current controller/filter boundary. |
| low | resolved hard-cut cleanup | Retired `suspend`, `unsuspend`, and `DELETE /api/v1/admin/users/{userId}` aliases are absent/non-mutating; canonical status, lock/unlock, session revocation, and MFA-disable routes remain. | `AdminUserController`, `IdentityRouteInventoryContractTest`, `openapi.json` | Client and worker guidance should continue using only the canonical route set to avoid restoring duplicate lifecycle semantics. |
