...
| Company directory and control plane | `GET /api/v1/companies`, `PUT /api/v1/companies/{id}` | `CompanyController` | List and update are real; destructive company delete is out of the live contract, and create moved to the super-admin onboarding flow only. |
