...
| The worker session started with unrelated modified files outside the docs-only write surface (`.env.example`, `production.env.template`, `.factory/**`, `docker-compose.yml`, and several `erp-domain/**` files). | `git status --porcelain` at worker startup (and current session status), plus mission `AGENTS.md` dirty-worktree guidance | Review synthesis changes had to be isolated carefully, and the pre-existing dirty tree reduced clean-room confidence for any repo-wide post-edit checks. |
