...
|---|---|---|---|---|
| 1 | Add tenant/company | Options: `GET /api/v1/superadmin/tenants/new`; template list: `GET /api/v1/superadmin/tenants/coa-templates`; create: `POST /api/v1/superadmin/tenants` | Draft or pending-activation tenant is created; owner credential setup happens through activation and `/api/v1/setup/**` | Invalid template code, duplicate tenant code/email, or activation delivery failure. Correct input or restore email delivery, then retry/resend activation. |
...
| 5 | Handle support interventions | Warning: `POST /api/v1/superadmin/tenants/{id}/support/warnings`; force logout: `POST /api/v1/superadmin/tenants/{id}/force-logout`; main-admin replacement: `PUT /api/v1/superadmin/tenants/{id}/admins/main`; admin email change: `POST /api/v1/superadmin/tenants/{id}/admins/{adminId}/email-change/{request|confirm}` | Support actions are persisted/audited and reflected back in the tenant detail support timeline | Wrong admin target, missing email verification token, or attempting to remove the current main admin before replacement. Confirm target identity and refresh tenant detail after action completion. |
...
- Support interventions: `POST /api/v1/superadmin/tenants/{id}/support/warnings`, `PUT /api/v1/superadmin/tenants/{id}/support/context`, `POST /api/v1/superadmin/tenants/{id}/force-logout`
