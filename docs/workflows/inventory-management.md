...
| 3 | Import opening stock at onboarding/restart for prepared SKUs only | **Opening Stock Import** → `POST /api/v1/inventory/opening-stock` (CSV file with explicit `Idempotency-Key` and `openingStockBatchKey`), history via `GET /api/v1/inventory/opening-stock` | Initial stock loads with row-level `results[]` and `errors[]`, plus per-SKU readiness details when a row is blocked | Missing key, missing `openingStockBatchKey`, orphan SKU, missing mirror/account readiness, CSV header mismatch, duplicate SKU/batch, or invalid date format. Fix readiness first; retry the same batch with the original `Idempotency-Key`, or use a new `Idempotency-Key` plus a new `openingStockBatchKey` only for a materially distinct follow-up import. |
