...
(
  ...
  MIGRATION_SET="$MIGRATION_SET" mvn -Djacoco.skip=true -Dtest='ProductionMonitoringContractTest,HealthEndpointProdHardeningIT,ActuatorProdHardeningIT' test
)

echo "[release-proof] targeted dispatch, accounting, and runtime boundary proof"
(
  cd "$ROOT_DIR/erp-domain"
  MIGRATION_SET="$MIGRATION_SET" mvn -Djacoco.skip=true -Dtest='DispatchControllerTest,DispatchOperationalBoundaryIT,DispatchConfirmationIT,TS_O2CDispatchCanonicalPostingTest,TS_O2COrchestratorDispatchRetirementTest,OrderFulfillmentE2ETest,JournalEntryE2ETest,AccountingEndpointContractTest,SettlementControllerIdempotencyHeaderParityTest,CriticalAccountingAxesIT,TS_RuntimeAccountingReplayConflictExecutableCoverageTest,ManualJournalSafetyTest,DealerReceiptSettlementAuditTrailTest,PayrollIdempotencyConcurrencyTest,PurchasingToApAccountingTest,SalesReturnCreditNoteIdempotencyTest,NumberSequenceServiceIntegrationTest,ReferenceNumberServiceTest,TS_RuntimeReferenceNumberServiceExecutableCoverageTest,OrderNumberServiceTest,InvoiceServiceTest,CompanyContextFilterControlPlaneBindingTest,AuthTenantAuthorityIT,TenantRuntimeEnforcementAuthIT,TS_RuntimeCompanyContextFilterExecutableCoverageTest,TS_RuntimeTenantRuntimeEnforcementTest,TS_RuntimeTenantControlPlaneEnforcementTest,CompanyControllerIT,OpenApiSnapshotIT' test
)
