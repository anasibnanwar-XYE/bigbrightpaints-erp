...
COORDINATOR_SUPPORT="$ROOT_DIR/erp-domain/src/main/java/com/bigbrightpaints/erp/orchestrator/service/IntegrationCoordinatorSupportService.java"
...
for path in "$DISPATCHER" "$COORDINATOR" "$COORDINATOR_SUPPORT" "$SANITIZER" "$CONTROLLER" "$IDEMPOTENCY_SERVICE" "$TRACE_SERVICE" "$EVENT_PUBLISHER" "$COORDINATOR_TEST" "$DISPATCHER_TEST" "$CONTROLLER_IT" "$SANITIZER_TEST"; do
...
require_multiline_pattern "reserveInventory\\([\\s\\S]*companyId,[\\s\\S]*traceId,[\\s\\S]*canonicalIdempotencyKey\\)" "$DISPATCHER" \
  "CommandDispatcher.approveOrder does not pass traceId/canonicalIdempotencyKey to reserveInventory"
require_multiline_pattern "updateFulfillment\\([\\s\\S]*companyId,[\\s\\S]*traceId,[\\s\\S]*canonicalIdempotencyKey\\)" "$DISPATCHER" \
  "CommandDispatcher.updateOrderFulfillment does not pass traceId/canonicalIdempotencyKey to updateFulfillment"
require_literal "CorrelationIdentifierSanitizer.sanitizeOptionalRequestId(requestId)" "$DISPATCHER" \
  "CommandDispatcher does not sanitize optional request identifiers via sanitizer contract"
...
require_literal "String canonicalIdempotencyKey = canonicalIdempotencyKey(lease);" "$DISPATCHER" \
  "CommandDispatcher must derive downstream idempotency from the leased command"
require_absent_literal "canonicalIdempotencyKey(lease, idempotencyKey)" "$DISPATCHER" \
  "CommandDispatcher must not fall back to the request idempotency key after lease creation"
require_absent_literal "resolveIdempotencyKey(" "$CONTROLLER" \
  "OrchestratorController must not derive idempotency keys from request IDs or payload hashes"
...
require_absent_literal "public void releaseInventory(" "$COORDINATOR" \
  "IntegrationCoordinator must not expose the removed releaseInventory batch shortcut"
...
require_literal "CorrelationIdentifierSanitizer.safeTraceForLog" "$COORDINATOR_SUPPORT" \
  "IntegrationCoordinatorSupportService logs are not using safe trace rendering"
require_literal "CorrelationIdentifierSanitizer.safeIdempotencyForLog" "$COORDINATOR_SUPPORT" \
  "IntegrationCoordinatorSupportService logs are not using safe idempotency rendering"
require_literal "CorrelationIdentifierSanitizer.sanitizeOptionalTraceId" "$COORDINATOR_SUPPORT" \
  "IntegrationCoordinatorSupportService memo/trace surfaces are not sanitized"
require_literal "CorrelationIdentifierSanitizer.sanitizeOptionalIdempotencyKey" "$COORDINATOR_SUPPORT" \
  "IntegrationCoordinatorSupportService memo/idempotency surfaces are not sanitized"
...
require_literal "CorrelationIdentifierSanitizer.sanitizeRequiredIdempotencyKey(idempotencyKey)" "$CONTROLLER" \
  "OrchestratorController does not require and sanitize Idempotency-Key header"
...
require_literal "CorrelationIdentifierSanitizer.sanitizeTraceIdOrGenerate" "$IDEMPOTENCY_SERVICE" \
